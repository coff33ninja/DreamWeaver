...
class TestServerAPI(unittest.TestCase):
        ...
        async def test_invalid_data():
            ...
            for invalid_request in invalid_requests:
                with self.subTest(request=invalid_request):
                    try:
                        response = await self.server_api.process_request(
                            invalid_request
                        )
    ...
    def test_error_stats_tracking(self):
        ...
        async def test_error_stats():
            ...
            with patch.object(
                self.server_api.request_handler,
                "handle_request",
...
class TestAPIResponse(unittest.TestCase):
    ...
    def test_response_to_dict(self):
        ...
        expected_dict = {"data": data, "status_code": 200, "message": "OK"}
...
class TestServerAPIIntegration(unittest.TestCase):
    ...
    def test_full_server_lifecycle(self):
        ...
        async def test_lifecycle():
            config = ServerConfig(
                {"host": "localhost", "port": 8081, "debug": True, "max_connections": 5}
            )
