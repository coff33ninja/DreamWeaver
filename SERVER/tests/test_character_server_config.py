...
class TestCharacterServerConfiguration:
    ...
    def test_character_server_with_environment_configuration(self):
        ...
        class ConfigurableServer(MockCharacterServer):
            ...
            def create_character(self, character_data):
                ...
                if self.enable_validation and not self.validate_character_data(
                    character_data
                ):
        ...
        with patch.dict(
            ...
            char_no_level = server2.create_character(
                {"name": "NoLevelHero", "class": "Mage"}
            )
            ...
            with pytest.raises(
                ValueError, match="Maximum number of characters reached"
            ):
                server2.create_character({"name": "Hero3", "class": "Rogue"})

            # Test disabled validation
            minimal_char = server2.create_character(
                {"name": "MinimalHero"}
    ...
    def test_character_server_with_json_configuration_file(self):
        ...
        config_data = {
            ...
            "character_defaults": {"hp": 100, "mp": 50, "status": "active"},
        }
        ...
        try:
            ...
            class ConfigFileServer(MockCharacterServer):
                def __init__(self, config_file):
                    ...
                    # Apply configuration
                    self.next_id = self.config["server_settings"][
                        "auto_increment_start"
                    ]
                    self.max_characters = self.config["server_settings"][
                        "max_characters"
                    ]
                ...
                def create_character(self, character_data):
                    ...
                    if (
                        len(name) < rules["min_name_length"]
                        or len(name) > rules["max_name_length"]
                    ):
            ...
            with pytest.raises(ValueError, match="Name length out of bounds"):
                server.create_character(
                    {"name": "X", "class": "Warrior", "level": 1}
                )  # Too short

            with pytest.raises(ValueError, match="Level out of bounds"):
                server.create_character(
                    {"name": "Hero", "class": "Warrior", "level": 150}
                )  # Too high
