...
class TestCharacterServerBenchmarks:
    ...
    def test_character_retrieval_performance_benchmark(self):
        ...
        character_ids = []
        for i in range(1000):
            char_data = {'name': f'RetrievalHero_{i}', 'class': 'Warrior', 'level': i % 100 + 1}
    ...
    def test_character_list_performance_scaling_benchmark(self):
        ...
        for size in dataset_sizes:
            ...
            while current_count < size:
                char_data = {'name': f'ListHero_{current_count}', 'class': 'Warrior', 'level': 1}
        ...
        for i in range(1, len(list_times)):
            ...
            assert time_ratio < size_ratio * 2, f"List performance scaling too poor: {time_ratio:.2f}x time for {size_ratio:.2f}x data"
    ...
    def test_mixed_operations_performance_benchmark(self):
        ...
        for i in range(num_operations):
            ...
            if operation == 'create':
                ...
                if all_chars:
                    char = random.choice(all_chars)
                    server.update_character(char['id'], {'level': random.randint(1, 100)})
        ...
        for op_type, stats in operation_stats.items():
            assert stats['avg_time'] < 0.01, f"{op_type} average time too slow: {stats['avg_time']:.6f}s"
            assert stats['max_time'] < 0.1, f"{op_type} maximum time too slow: {stats['max_time']:.6f}s"
