...
def run_all_tests():
    """
    Runs all CSM test classes and returns whether all tests passed.
    
    Returns:
        bool: True if all tests succeed, False otherwise.
    """
    # Create test suite
...
def run_specific_test(test_class_name):
    """
    Run tests from a specific test class by name.
    
    Parameters:
        test_class_name (str): The name of the test class to run (case-insensitive). Must match one of the predefined test class keys.
    
    Returns:
        bool: True if all tests in the specified class pass, False otherwise or if the class name is unknown.
    """
    test_classes = {
