...
async def save_training_data(
    ...
    """
    Saves a training dataset for a specified actor after validating authentication.
    
    Validates the provided token for the given actor, then stores the training data in the database. Returns a success message upon completion.
    
    Raises:
        HTTPException: If the token is invalid (401) or if saving the data fails (500).
    """
...
async def register_client_endpoint( # Renamed to avoid conflict with db.register_client
    ...
    """
    Registers or updates a client in the database with the provided Actor ID, client IP address, and port.
    
    Raises:
        HTTPException: If the provided token is invalid (401) or registration fails (500).
...
async def client_heartbeat(
    ...
    """
    Handle client heartbeat updates by validating the token and recording the client's online status and last-seen timestamp.
    
    Returns:
        dict: Confirmation message indicating the heartbeat was received and the client is marked as online.
    
    Raises:
        HTTPException: If the token is invalid (401) or if an error occurs during the update (500).
    """
    if not client_manager.validate_token(request_data.Actor_id, request_data.token):
...
async def get_reference_audio(
    ...
    Clients must provide a valid `Actor_id` and `token` as query parameters. The function checks for path traversal attempts and ensures the requested file exists within the allowed reference audio directory. Returns the audio file as a WAV response if all checks pass.
    """
