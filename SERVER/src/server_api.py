...
async def get_traits(
    ...
    character = db.get_character(
        Actor_id
    )  # Assumes Actor_id is the primary key for characters
    if not character:
        logger.warning(f"Character not found for Actor_id: {Actor_id} in get_traits.")
        raise HTTPException(
            status_code=404, detail=f"Character for Actor ID '{Actor_id}' not found"
        )
...
async def save_training_data(
    ...
    if not client_manager.authenticate_request(
        ...
        logger.warning(
            f"Authentication failed for Actor_id: {request_data.Actor_id} in save_training_data."
        )
        raise HTTPException(status_code=401, detail="Invalid or expired token")
    ...
    try:
        ...
        return {
            "message": f"Training data for {request_data.Actor_id} saved successfully"
        }
    except Exception as e:
        logger.error(
            f"Failed to save training data for {request_data.Actor_id}: {e}",
            exc_info=True,
        )
        raise HTTPException(
            status_code=500,
            detail="An internal error occurred while saving training data.",
        )
...
async def register_client_endpoint(  # Renamed to avoid conflict with db.register_client
    ...
    if not client_manager.validate_token(request_data.Actor_id, request_data.token):
        logger.warning(
            f"Invalid primary token for Actor_id: {request_data.Actor_id} during registration from IP: {client_ip}."
        )
        raise HTTPException(
            status_code=401, detail="Invalid primary token for registration."
        )
    ...
    try:
        ...
        logger.info(
            f"Client {request_data.Actor_id} registered from {client_ip}:{request_data.client_port}. Status: Online."
        )
        return {
            "message": f"Client {request_data.Actor_id} registered from {client_ip}:{request_data.client_port}. Status: Online."
        }
    except Exception as e:
        logger.error(
            f"Failed to register client {request_data.Actor_id} from IP {client_ip}: {e}",
            exc_info=True,
        )
        raise HTTPException(
            status_code=500,
            detail="An internal error occurred during client registration.",
        )
...
async def client_heartbeat(
    ...
    if not client_manager.authenticate_request(
        ...
        logger.warning(
            f"Authentication failed for Actor_id: {request_data.Actor_id} in heartbeat."
        )
        raise HTTPException(
            status_code=401, detail="Invalid or expired token for heartbeat"
        )
    ...
    try:
        ...
        return {
            "message": f"Heartbeat received from {request_data.Actor_id}. Status: Online."
        }
    except Exception as e:
        logger.error(
            f"Error processing heartbeat for {request_data.Actor_id}: {e}",
            exc_info=True,
        )
        raise HTTPException(
            status_code=500,
            detail="An internal error occurred while processing heartbeat.",
        )


@app.get(
    "/get_reference_audio/{filename}",
    summary="Download Reference Audio for Voice Cloning",
)
async def get_reference_audio(
    filename: str,
    Actor_id: str,  # Added Actor_id as query param
    token: str,  # Added token as query param
    client_manager: ClientManager = Depends(
        get_client_manager
    ),  # No db needed if just validating token
):
    """
    ...
    if not client_manager.authenticate_request(Actor_id, token):
        ...
        raise HTTPException(
            status_code=401, detail="Invalid or expired token for audio download"
        )
    ...
    if safe_name != filename:
        logger.error(
            f"Potential path traversal attempt for filename: {filename} by Actor_id: {Actor_id}."
        )
    ...
    if not os.path.exists(file_path) or not os.path.isfile(file_path):
        logger.warning(
            f"Reference audio file '{filename}' not found for Actor_id: {Actor_id}."
        )
        raise HTTPException(
            status_code=404, detail=f"Reference audio file '{filename}' not found."
        )

    # Ensure the file is within the allowed directory (important security check)
    if not os.path.abspath(file_path).startswith(
        os.path.abspath(REFERENCE_VOICES_AUDIO_PATH)
    ):
        logger.error(
            f"Forbidden access attempt to file path: {file_path} by Actor_id: {Actor_id}."
        )
        raise HTTPException(
...
@app.get(
    "/download_client_config/{actor_id}", summary="Download Client Configuration File"
)
async def download_client_config(
    ...
    """
    token_details = db.get_client_token_details(
        actor_id
    )  # Fetch full details to ensure actor exists
    if not token_details or not token_details.get("token"):
        logger.warning(
            f"Token not found for Actor_id: {actor_id} in download_client_config."
        )
        raise HTTPException(
            status_code=404,
            detail=f"Token for Actor ID '{actor_id}' not found or actor does not exist.",
        )

    token = token_details["token"]
    logger.info(
        f"Generating client config for Actor_id: {actor_id} with server_url: {server_url}."
    )
...
@app.post(
    "/request_handshake_challenge", summary="Request a challenge for session handshake"
)
async def request_handshake_challenge(
    ...
    if not client_manager.validate_token(Actor_id, client_primary_token):
        logger.warning(
            f"Handshake: Invalid primary token for Actor_id: {Actor_id} during challenge request."
        )
        raise HTTPException(
...
@app.post(
    "/submit_handshake_response",
    summary="Submit challenge response to get a session token",
)
async def submit_handshake_response(
    ...
    if expected_response_hash == client_challenge_response:
        ...
        try:
            ...
            logger.info(
                f"Handshake: Issued session token for Actor_id: {Actor_id}, expires at {session_expiry.isoformat()}"
            )
            return {
            ...
            logger.error(
                f"Handshake: Failed to save session token for Actor_id {Actor_id}: {e}",
                exc_info=True,
            )
...
async def websocket_endpoint(
    ...
    if not session_token:
        logger.warning(
            f"WebSocket: Connection attempt from Actor_id {actor_id} without a session token."
        )
        await websocket.close(
            code=4001, reason="Session token required"
        )  # Custom close code
        return
    ...
    if not client_manager_dep.authenticate_request(actor_id, session_token):
        ...
        await websocket.close(
            code=4003, reason="Invalid session token"
        )  # Custom close code
        return
    ...
    try:
        ...
        logger.info(
            f"WebSocket: Client {actor_id} disconnected (WebSocketDisconnect received)."
        )
    except Exception as e:
        logger.error(
            f"WebSocket: Unexpected error with client {actor_id} in main loop: {e}",
            exc_info=True,
        )
        if not websocket.client_state == websockets.protocol.State.CLOSED:  # type: ignore
            try:
                ...
                logger.info(
                    f"WebSocket: Closed connection to {actor_id} due to server-side exception."
                )
            except Exception as e_close:
                logger.error(
                    f"WebSocket: Error trying to close connection with {actor_id} after exception: {e_close}",
                    exc_info=True,
                )
    finally:
