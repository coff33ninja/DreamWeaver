...
class WebSocketConnectionManager:
    ...
    async def connect(self, websocket: WebSocket, actor_id: str):
        ...
        logger.info(
            f"WebSocket connection established for Actor_id: {actor_id}. Total connections: {len(self.active_connections)}"
        )
    ...
    def disconnect(self, actor_id: str):
        ...
        if actor_id in self.active_connections:
            ...
            logger.info(
                f"WebSocket connection closed for Actor_id: {actor_id}. Total connections: {len(self.active_connections)}"
            )
        else:
            logger.warning(
                f"Attempted to disconnect Actor_id: {actor_id}, but no active WebSocket connection found."
            )
    ...
    async def send_personal_message(self, message: dict, actor_id: str) -> bool:
        ...
        if websocket := self.active_connections.get(actor_id):
            try:
                ...
                logger.info(
                    f"Sent WebSocket message to Actor_id {actor_id}: {str(message)[:100]}..."
                )
                return True
            except (
                Exception
            ) as e:  # Handles if websocket is already closed unexpectedly etc.
                logger.error(
                    f"Error sending WebSocket message to Actor_id {actor_id}: {e}",
                    exc_info=True,
                )
            ...
            logger.warning(
                f"No active WebSocket connection found for Actor_id {actor_id} to send message: {str(message)[:100]}..."
            )
            return False
    ...
    async def broadcast(self, message: dict):  # Not used initially, but good to have
        ...
        """
        logger.info(
            f"Broadcasting WebSocket message to {len(self.active_connections)} clients: {str(message)[:100]}..."
        )
        ...
        for actor_id, websocket in self.active_connections.items():
            try:
                ...
                logger.error(
                    f"Error broadcasting to Actor_id {actor_id}: {e}", exc_info=True
                )
