...
def update_model_dropdown(service_name: str):
    ...
    Returns:
        dict: A dictionary with 'choices' as the list of available models and 'value' as the default selected model, or None if no models are available.
    """
...
async def get_story_playback_data_async():
    ...
    Returns:
        list: A list of message dictionaries with roles and formatted content for the chatbot. If no history is found, returns a system message indicating the absence of story history.
    """
...
async def create_character_async(name, personality, goals, backstory, tts_service, tts_model, reference_audio_file, Actor_id, progress=gr.Progress(track_tqdm=True)):
    """
    Asynchronously creates or updates a character with the specified attributes and optional reference audio, saving details to the database and generating an access token for non-primary actors.
    
    If the TTS service is "xttsv2" and a reference audio file is provided, the audio is saved to the configured directory with a sanitized filename. If "xttsv2" is selected but no reference audio is uploaded, an error message is returned. After saving the character, a token is generated for actors other than "Actor1".
    ...
    Parameters:
        ...
        reference_audio_file (UploadedFile or None): Reference audio file for XTTSv2 voices, if applicable.
...
async def story_interface_async(audio_input_path, chaos_level_value, progress=gr.Progress(track_tqdm=True)):
    ...
    Parameters:
    	...
    	chaos_level_value: Value controlling the randomness or variability in story generation.
    
    Returns:
    	narration (str): Generated narration text.
    	character_texts (dict): Dictionary of character dialogue outputs keyed by character name. If an error occurs, returns an error message and empty outputs.
    """
...
async def export_story_async(export_format, progress=gr.Progress(track_tqdm=True)):
    ...
    Parameters:
        export_format (str): The desired export format (e.g., "text", "json").
...
async def save_env_vars_async(new_vars_str: str, progress=gr.Progress()):
    """
    Asynchronously saves updated environment variables to the .env file and refreshes the displayed variables.
    ...
    Returns:
        ...
        new_status (str): The status message after refreshing environment variables.
        new_vars_display (str): The masked string representation of the updated environment variables.
    """
...
async def set_api_provider_async(selected_provider, progress=gr.Progress()):
    """
    Asynchronously updates the API provider in the environment variables and refreshes the displayed environment variable status.
    ...
    Returns:
    	...
    	new_status (str): Updated status of environment variables.
    	new_vars_display (str): Masked string representation of current environment variables.
...
async def restart_server_async(progress=gr.Progress()):
    """
    Restart the server process asynchronously by re-executing the current Python interpreter.
    
    This function triggers a full process restart, which is required for certain configuration or environment changes to take effect. The function does not return to the caller, as the process is replaced.
    """
...
def launch_interface():
    """
    Launches the Dream Weaver Gradio web interface, initializing all core components and defining the full multi-tab asynchronous UI for character management, story narration, playback, checkpointing, environment variable editing, and configuration.
    
    This function sets up global instances for the database, client manager, story manager, checkpoint manager, and environment manager, then constructs a Gradio Blocks application with multiple interactive tabs. Each tab provides specialized controls and async event handlers for managing characters, narrating and correcting story progression, reviewing story history, handling checkpoints and exports, editing environment variables and API keys, and updating configuration options. The interface supports progress tracking, dynamic UI updates, and server restart functionality.
    
    The Gradio app is launched with async queuing enabled, binding to all network interfaces on port 7860.
    """
        ...
        with gr.Tabs():
            ...
            with gr.TabItem("Story Progression"):
                ...
                async def save_correction_async(correction_text, progress=gr.Progress(track_tqdm=True)):
                    """
                    Persist a corrected narration text to the database asynchronously.
            ...
            with gr.TabItem("Story Playback & History"):
                ...
                async def refresh_story_async_wrapper(progress=gr.Progress()):
                    ...
                    Returns:
                        data (list): The formatted story history data for display.
                    """
            ...
            with gr.TabItem("API Keys & .env"):
                ...
                def get_token_field(provider):
                    ...
                    Returns:
                    	dict: A dictionary specifying the field's visibility, label, placeholder, and initial value.
                    """
                ...
                def hide_token_field():
                    """
                    Hide the token input field in the UI by returning a visibility configuration.
                    """
                ...
                def update_token_field(provider):
                    ...
                    Returns:
                        dict: A dictionary specifying the visibility, label, placeholder, and value for the token input field.
                    """
                ...
                async def save_token_async(provider, token, progress=gr.Progress()):
                    ...
                    Returns:
                        ...
                        new_status (str): The updated status message for environment variables.
                        new_vars_display (str): The masked display of environment variables.
