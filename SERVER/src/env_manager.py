...
def save_env_vars(new_vars_str: str):
    ...
    try:
        ...
        logger.info(
            f"Saved variables to {ENV_FILE_PATH}. New content (masked for sensitive keys): {load_env_vars(mask_sensitive=True)}"
        )
        return msg
    except Exception as e:
        logger.error(
            f"Error saving to .env file at {ENV_FILE_PATH}: {e}", exc_info=True
        )
