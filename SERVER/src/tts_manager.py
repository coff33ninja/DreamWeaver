...
class TTSManager:
    ...
    def _initialize_service(self):
        """
        Initializes the TTS service by loading the required models and setting up the backend for synthesis.
        
        This method performs blocking initialization based on the selected TTS service. For "gtts", it verifies the library is available and sets up the blocking synthesis method. For "xttsv2", it checks for a valid model name, ensures the model is available, and attempts to instantiate the Coqui TTS model on the appropriate device. Prints warnings or errors if initialization fails or if the service is unsupported.
        """
    ...
    def _xttsv2_synthesize_blocking(self, text: str, output_path: str, speaker_wav: Optional[str] = None, lang: str = "en"):
        ...
        If a valid speaker WAV file is provided and exists, it is used for voice cloning; otherwise, the default voice is used. The language is set to the specified value if supported by the model, or defaults to the first available language.
    ...
    def _get_or_download_model_blocking(self, service_name, model_identifier):
        ...
        For the "xttsv2" service, returns the provided model identifier directly, as model management is handled internally by Coqui TTS. Returns None for other services.
        """
    ...
    def list_services():
        ...
        Returns:
            services (list of str): Names of supported TTS services, such as "gtts" and/or "xttsv2".
        """
        services = []
        if gtts:
...
if __name__ == "__main__":
    async def test_tts_manager():
        """
        Asynchronously tests the TTSManager with available TTS services by synthesizing sample audio files.
        
        This function creates a test output directory and, for each supported TTS service ("gtts" and/or "xttsv2"), initializes a TTSManager instance, performs asynchronous synthesis of a sample phrase, and saves the resulting audio file. Status messages are printed to indicate progress and output locations.
        """
