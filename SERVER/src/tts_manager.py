...
class TTSManager:
    def __init__(
        ...
        os.makedirs(
            os.path.join(MODELS_PATH, "tts_models"), exist_ok=True
        )  # Common Coqui subfolder

        logger.info(
            f"Server TTSManager: Initializing for service '{self.service_name}' with model '{self.model_name}', lang '{self.language}', speaker_wav: '{self.speaker_wav_path}'"
        )
        ...
        if self.service_name == "xttsv2":
            if CoquiTTS:
                try:
                    ...
                    logger.info(
                        f"Server TTSManager: Initializing Coqui XTTSv2 with model: {model_path_or_name or self.model_name}"
                    )
                    ...
                    self.is_initialized = True
                    logger.info(
                        f"Server TTSManager: Coqui XTTSv2 initialized: {model_path_or_name or self.model_name}. Device: {device}"
                    )
                except Exception as e:
                    logger.error(
                        f"Server TTSManager: Error initializing Coqui XTTSv2 model {model_path_or_name or self.model_name}: {e}",
                        exc_info=True,
                    )
            else:
                logger.error(
    ...
    def _xttsv2_synthesize_blocking(
        ...
        """
        if (
            not self.tts_instance
            or callable(self.tts_instance)
            or not hasattr(self.tts_instance, "tts_to_file")
        ):
            logger.error(
                "Server TTSManager: XTTSv2 instance is not initialized or invalid for _xttsv2_synthesize_blocking."
            )
            return
        ...
        if (
            ...
            logger.debug(
                f"Server TTSManager: Synthesizing XTTSv2 with speaker_wav: {speaker_to_use}, lang: {lang_to_use}"
            )
            ...
            if (
                speaker_to_use
            ):  # Only log warning if a speaker_wav was intended but not found
                logger.warning(
                    f"Server TTSManager: XTTSv2 speaker_wav '{speaker_to_use}' not found or invalid. Using default voice for lang {lang_to_use}."
                )
            else:
                logger.debug(
                    f"Server TTSManager: Synthesizing XTTSv2 with default voice for lang {lang_to_use}."
                )
            self.tts_instance.tts_to_file(
                text=text, language=lang_to_use, file_path=output_path
            )
    ...
    async def synthesize(
        ...
        if not self.is_initialized or not self.tts_instance:
            logger.error(
                f"Server TTSManager ({self.service_name}): Not initialized, cannot synthesize text: '{text[:50]}...'."
            )
        ...
        if not os.path.exists(output_dir):
            try:
                ...
                logger.error(
                    f"Server TTSManager: Could not create output directory {output_dir}: {e}",
                    exc_info=True,
                )
                return False

        logger.info(
            f"Server TTSManager: Synthesizing text '{text[:50]}...' to {output_path} using {self.service_name}"
        )
        try:
            if self.service_name == "gtts":
                ...
                logger.error(
                    f"Server TTSManager: No async synthesis method for unsupported service '{self.service_name}'."
                )
                raise ValueError(f"Unsupported TTS service: {self.service_name}")

            if os.path.exists(output_path) and os.path.getsize(output_path) > 0:
                logger.info(
                    f"Server TTSManager: Successfully synthesized audio to {output_path}"
                )
                return True
            else:
                logger.error(
                    f"Server TTSManager: Synthesis completed but output file {output_path} is missing or empty."
                )
                return False
        except Exception as e:
            logger.error(
                f"Server TTSManager: Error during async TTS synthesis with {self.service_name} for text '{text[:50]}...': {e}",
                exc_info=True,
            )
            if os.path.exists(output_path):  # Attempt to clean up failed/partial file
                try:
                    ...
                    logger.info(
                        f"Server TTSManager: Removed partial/failed output file {output_path}"
                    )
                except OSError as e_remove:
                    logger.error(
                        f"Server TTSManager: Error removing partial/failed output file {output_path}: {e_remove}",
                        exc_info=True,
                    )
            return False
