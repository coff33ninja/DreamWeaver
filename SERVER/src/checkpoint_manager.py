...
class CheckpointManager:
    def __init__(self, server_model_name="TinyLLaMA", server_Actor_id="Actor1"):
        ...
        try:
            ...
            logger.info(f"CheckpointManager initialized. Checkpoint path: {BASE_CHECKPOINT_PATH}, Server adapter path: {self.server_adapter_specific_path}")
        except OSError as e:
            logger.error(f"Error creating directories during CheckpointManager initialization: {e}", exc_info=True)
    ...
    def list_checkpoints(self):
        ...
        try:
            checkpoints = sorted(
                [d for d in os.listdir(BASE_CHECKPOINT_PATH) if os.path.isdir(os.path.join(BASE_CHECKPOINT_PATH, d))],
                key=lambda x: os.path.getmtime(os.path.join(BASE_CHECKPOINT_PATH, x)), # Sort by modification time
                reverse=True
            )
            logger.debug(f"Found checkpoints: {checkpoints}")
            return checkpoints
        except FileNotFoundError:
            logger.warning(f"Checkpoint directory {BASE_CHECKPOINT_PATH} not found when listing checkpoints.")
            return []
    ...
    def save_checkpoint(self, name_prefix=""):
        ...
        logger.info(f"Attempting to save checkpoint: {checkpoint_name} to {checkpoint_dir}")
        ...
        try:
            ...
            if os.path.exists(DB_PATH): # Use DB_PATH from config
                ...
                logger.warning(f"Database file not found at {DB_PATH}. Skipping DB save for checkpoint {checkpoint_name}.")
            ...
            if os.path.exists(self.server_adapter_specific_path): # Use SERVER_ADAPTER_SPECIFIC_PATH
                ...
                logger.info(f"Server LLM adapters for {self.server_Actor_id} saved to checkpoint {checkpoint_name}")
            else:
                logger.warning(f"Server LLM adapters not found at {self.server_adapter_specific_path}. Skipping adapter save for checkpoint {checkpoint_name}.")

            return f"Checkpoint '{checkpoint_name}' saved successfully.", self.list_checkpoints()
        except Exception as e:
            logger.error(f"Error saving checkpoint {checkpoint_name}: {e}", exc_info=True)
            # Attempt to clean up partially created checkpoint directory
            if os.path.isdir(checkpoint_dir):
                try:
                    shutil.rmtree(checkpoint_dir)
                    logger.info(f"Cleaned up partially created checkpoint directory: {checkpoint_dir}")
                except Exception as cleanup_e:
                    logger.error(f"Error cleaning up checkpoint directory {checkpoint_dir}: {cleanup_e}", exc_info=True)
    ...
    def load_checkpoint(self, checkpoint_name):
        ...
        """
        checkpoint_dir = os.path.join(BASE_CHECKPOINT_PATH, checkpoint_name)
        logger.info(f"Attempting to load checkpoint: {checkpoint_name} from {checkpoint_dir}")
        ...
        try:
            ...
            if os.path.exists(db_in_checkpoint):
                ...
                logger.error(f"Database file not found in checkpoint '{checkpoint_name}'.")
            ...
            if os.path.exists(adapters_in_checkpoint):
                if os.path.exists(self.server_adapter_specific_path): # Use SERVER_ADAPTER_SPECIFIC_PATH
                    logger.info(f"Removing existing server adapters at {self.server_adapter_specific_path} before restoring from checkpoint.")
                ...
                logger.info(f"Server LLM adapters for {self.server_Actor_id} restored from checkpoint {checkpoint_name}")
            else:
                logger.warning(f"No {self.server_Actor_id} adapters found in checkpoint '{checkpoint_name}'. Skipping adapter load.")

            return f"Checkpoint '{checkpoint_name}' loaded. PLEASE RESTART THE APPLICATION for changes to take effect."
        except Exception as e:
            logger.error(f"Error loading checkpoint {checkpoint_name}: {e}", exc_info=True)
    ...
    def export_story(self, export_format="text"):
        ...
        from .database import Database # Local import to avoid circular dependency issues with logging setup
        db = Database(DB_PATH) # Use DB_PATH from config
        history = db.get_story_history() # This method in DB should ideally use its own logger
        ...
        try:
            ...
            logger.error(f"Error creating export directory {export_dir}: {e}", exc_info=True)
        ...
        if export_format == "json":
            ...
            story_data = [{"speaker": entry["speaker"], "text": entry["text"], "timestamp": entry["timestamp"]} for entry in history]
            try:
                ...
                logger.error(f"Error exporting story to JSON file {export_path}: {e}", exc_info=True)
            ...
            try:
                with open(export_path, "w", encoding="utf-8") as f:
                    for entry in history: # Assuming history items are dicts or sqlite3.Row
                ...
                logger.error(f"Error exporting story to Text file {export_path}: {e}", exc_info=True)
