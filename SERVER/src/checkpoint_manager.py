...
class CheckpointManager:
    def __init__(self, server_model_name="TinyLLaMA", server_Actor_id="Actor1"):
        ...
        Creates necessary directories for checkpoints and server-specific adapters based on the provided model name and actor ID.
        """
    ...
    def list_checkpoints(self):
        """
        Return a list of available checkpoint directory names, sorted with the most recent first.
        
        Returns:
            list[str]: List of checkpoint directory names, or an empty list if no checkpoints exist.
        """
        try:
    ...
    def save_checkpoint(self, name_prefix=""):
        """
        Create a new checkpoint by saving the current database and server-specific adapters to a timestamped directory.
    ...
    def load_checkpoint(self, checkpoint_name):
        ...
        Returns:
        	str: A message indicating success, error, or required actions after restoration.
        """
        checkpoint_dir = os.path.join(BASE_CHECKPOINT_PATH, checkpoint_name)
    ...
    def export_story(self, export_format="text"):
        ...
        Parameters:
            export_format (str): The format for export, either "text" or "json". Defaults to "text".
        
        Returns:
            tuple: A tuple containing a message string and the export filename (or None on error).
        """
