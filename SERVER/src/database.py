...
class Database:
    ...
    def _get_conn(self):
        """
        Returns a SQLite database connection unique to the current thread, creating it if necessary.
        
        If a connection does not already exist for the thread, a new one is established and configured to return rows as dictionaries.
        """
    ...
    def _execute_query(self, query, params=None, commit=False, fetchone=False, fetchall=False):
        """
        Executes a SQL query with optional parameters and supports commit and fetch operations.
        ...
        Returns:
            The fetched row, list of rows, cursor object, or None if an error occurs.
        """
        conn = self._get_conn()
        try:
            cursor = conn.cursor()
    ...
    def _ensure_column(self, table_name, column_name, column_definition):
        """
        Ensures that a specified column exists in a database table, adding it if missing.
    ...
    def _ensure_schema(self):
        """
        Create all required database tables and ensure that necessary columns exist for schema compatibility.
        
        This method creates the `characters`, `story_log`, `training_data`, and `client_tokens` tables if they do not already exist, and adds any missing columns needed for new features or schema evolution. It also enforces foreign key relationships and sets default values where appropriate.
        """
        self._execute_query("""
    ...
    def save_character(self, name, personality, goals, backstory, tts, tts_model, reference_audio_filename, Actor_id, llm_model=None):
        """
        Insert a new character or update an existing character's details in the database using the provided Actor_id.
        
        If a character with the given Actor_id already exists, their attributes are updated; otherwise, a new character record is created.
        """
        query = """
    ...
    def get_character(self, Actor_id):
        ...
        """
        Retrieve a character's details by their Actor_id.
        
        Returns:
            dict: A dictionary containing the character's attributes if found, otherwise None.
        """
    ...
    def save_story(self, narration_text, character_texts, narrator_audio_path=None):
        """
        Saves a story progression by recording a narrator entry and corresponding character entries in the story log.
        
        Parameters:
            narration_text (str): The narration text to be saved as the narrator's entry.
            character_texts (dict): A mapping of character names to their respective dialogue or text.
    ...
    def get_story_history(self):
        """
        Retrieve the complete story history as a list of entries ordered by timestamp.
        
        Returns:
            List[dict]: A list of dictionaries, each representing a story log entry with keys: 'id', 'speaker', 'text', 'timestamp', and 'narrator_audio_path'. Returns an empty list if no entries exist.
        """
    ...
    def save_training_data(self, dataset, Actor_id):
        """
        Insert a new training data entry linked to the specified Actor.
        
        Parameters:
            dataset (dict): Dictionary containing 'input' and 'output' text fields.
            Actor_id (str): Unique identifier for the character associated with the training data.
        """
    ...
    def get_training_data_for_Actor(self, Actor_id):
        """
        Retrieve all training data entries associated with a given Actor.
        
        Parameters:
            Actor_id (str): The unique identifier for the Actor whose training data is to be retrieved.
        
        Returns:
            List[dict]: A list of dictionaries, each containing 'input' and 'output' keys representing the input and output text for the Actor. Returns an empty list if no data is found.
        """
    ...
    def save_client_token(self, Actor_id, token):
        ...
        If the Actor_id is "Actor1" and no corresponding character exists, a placeholder character is created automatically.
        """
    ...
    def get_client_token_details(self, Actor_id):
        """
        Retrieve the token details and client status information for a given Actor_id.
    ...
    def get_token(self, Actor_id):
        """
        Retrieve the token string associated with the specified Actor_id.
    ...
    def register_client(self, Actor_id, ip_address, client_port):
        """
        Registers or updates a client's network information and marks it as online.
        
        Sets the client's IP address, port, and last seen timestamp, and updates its status to 'Online_Heartbeat'.
        """
    ...
    def update_client_status(self, Actor_id, new_status, last_seen_iso=None):
        """
        Update the status and last seen timestamp for a client identified by Actor_id.
        
        If last_seen_iso is not provided, the current UTC time is used.
        """
    ...
    def get_clients_for_story_progression(self):
        """
        Retrieve a list of clients that are currently responsive and eligible for story progression.
        
        Returns:
            List[dict]: A list of dictionaries containing `Actor_id`, `ip_address`, and `client_port` for each client with status 'Online_Responsive' and a recent heartbeat, excluding 'Actor1'.
        """
    ...
    def get_all_client_statuses(self):
        """
        Retrieve the status and connection details of all clients except "Actor1".
        
        Returns:
            List[dict]: A list of dictionaries containing `Actor_id`, `ip_address`, `client_port`, `last_seen`, and `status` for each client, ordered by `Actor_id`. Returns an empty list if no clients are found.
        """
    ...
    def close(self):
        """
        Closes the current thread's database connection if it exists, releasing associated resources.
        """
    ...
    def update_story_entry(self, entry_id, new_text=None, new_audio_path=None):
        """
        Update the text and/or narrator audio path of a story_log entry identified by its ID.
        
        Parameters:
            entry_id (int): The unique identifier of the story_log entry to update.
            new_text (str, optional): The new text to set for the entry.
            new_audio_path (str, optional): The new narrator audio path to set for the entry.
        
        Returns:
            bool: True if at least one field was updated, False if no update was performed.
        """
        sets = []
        params = []
