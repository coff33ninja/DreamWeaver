...
class Database:
    def __init__(self, db_path):
        """
        Initialize the Database object with the specified SQLite file path, set up thread-local storage for connections, and ensure the required database schema exists.
        """
    ...
    def _get_conn(self):
        """
        Returns a SQLite database connection specific to the current thread, creating one if it does not already exist.
    ...
    def _execute_query(self, query, params=None, commit=False, fetchone=False, fetchall=False):
        """
        Execute a SQL query with optional parameters and support for committing transactions and fetching results.
        
        Parameters:
            query (str): The SQL query to execute.
            params (tuple or list, optional): Parameters to substitute into the query.
            commit (bool, optional): Whether to commit the transaction after executing the query.
            fetchone (bool, optional): If True, return a single result row as a dictionary.
            fetchall (bool, optional): If True, return all result rows as a list of dictionaries.
        
        Returns:
            The query result as a dictionary (if fetchone), a list of dictionaries (if fetchall), the cursor object (if neither fetchone nor fetchall), or None if an error occurs.
        """
        conn = self._get_conn()
        try:
            cursor = conn.cursor()
    ...
    def _ensure_schema(self):
        """
        Creates all required tables and ensures necessary columns exist in the database schema for characters, story logs, training data, and client tokens.
        
        This method creates tables if they do not exist and adds missing columns to support new features or schema changes, including character-specific LLM models, narrator audio paths, and detailed client status tracking. Commits all schema changes to the database.
        """
        self._execute_query("""
    ...
    def save_character(self, name, personality, goals, backstory, tts, tts_model, reference_audio_filename, Actor_id, llm_model=None):
        """
        Insert a new character or update an existing character's record in the database using the provided attributes.
        
        If a character with the given Actor_id already exists, its details are updated; otherwise, a new record is created.
        """
        query = """
    ...
    def get_character(self, Actor_id):
        ...
        """
        Retrieve a character's full record by Actor_id.
        
        Returns:
            dict: A dictionary containing the character's attributes if found, or None if no matching record exists.
        """
    ...
    def save_story(self, narration_text, character_texts, narrator_audio_path=None):
        """
        Saves a story segment by recording the narrator's entry and each character's contribution.
        
        Parameters:
            narration_text (str): The text narrated by the narrator.
            character_texts (dict): A mapping of character names to their respective dialogue text.
    ...
    def get_story_history(self):
        ...
        Returns:
            List[dict]: A list of dictionaries, each representing a story log entry with keys 'id', 'speaker', 'text', 'timestamp', and 'narrator_audio_path'. Returns an empty list if no entries are found.
        """
    ...
    def save_training_data(self, dataset, Actor_id):
        """
        Insert a new training data entry linked to a specific actor.
        
        Parameters:
            dataset (dict): A dictionary containing 'input' and 'output' text fields.
            Actor_id (str): The unique identifier for the actor associated with the training data.
        """
    ...
    def get_training_data_for_Actor(self, Actor_id):
        """
        Retrieve all training data entries associated with a given Actor_id.
        
        Parameters:
            Actor_id (str): The unique identifier for the actor whose training data is requested.
        
        Returns:
            List[dict]: A list of dictionaries, each containing 'input' and 'output' keys for the training data pairs. Returns an empty list if no data is found.
        """
    ...
    def register_client(self, Actor_id, ip_address, client_port):
        ...
        Sets the client's IP address, port, and last seen timestamp in the database, and updates its status to 'Online_Heartbeat'.
        """
    ...
    def get_clients_for_story_progression(self):
        """
        Retrieve a list of clients that are currently responsive and eligible for story progression, excluding the primary narrator client.
        
        Returns:
            A list of dictionaries, each containing the Actor ID, IP address, and client port for clients with status 'Online_Responsive' and recent activity.
        """
    ...
    def get_all_client_statuses(self):
        """
        Retrieve the status and network information for all clients except 'Actor1'.
        
        Returns:
            A list of dictionaries, each containing 'Actor_id', 'ip_address', 'client_port', 'last_seen', and 'status' for each client.
        """
    ...
    def close(self):
        """
        Closes the current thread's SQLite database connection if it exists and removes the reference.
        """
    ...
    def update_story_entry(self, entry_id, new_text=None, new_audio_path=None):
        """
        Update the text and/or narrator audio path of a story log entry identified by its ID.
