...
class Database:
    def __init__(self, db_path):
        """
        Initialize the Database instance with the specified SQLite file path, setting up thread-local storage for connections and ensuring the required database schema exists.
        """
    ...
    def _get_conn(self):
        """
        Returns the SQLite database connection specific to the current thread, creating it if it does not already exist.
        
        If the connection cannot be established, raises a `sqlite3.Error`.
        """
    ...
    def _execute_query(self, query, params=None, commit=False, fetchone=False, fetchall=False):
        ...
        Returns:
            The query result as a dictionary, list of dictionaries, or cursor object depending on fetch options, or None if an error occurs.
        """
        conn = self._get_conn()
        try:
            cursor = conn.cursor()
    ...
    def _ensure_column(self, table_name, column_name, column_definition):
        """
        Ensures that a specified column exists in a given table, adding it if missing.
        
        If the column does not exist in the table, it is added using the provided column definition.
        """
        try:
    ...
    def _ensure_schema(self):
        """
        Ensures that all required database tables and columns exist, creating or updating them as necessary for application operation.
        
        This method creates the `characters`, `story_log`, `training_data`, and `client_tokens` tables if they do not already exist, and adds any missing columns needed for new features or schema evolution. It also commits the schema changes to the database.
        """
        self._execute_query("""
    ...
    def save_character(self, name, personality, goals, backstory, tts, tts_model, reference_audio_filename, Actor_id, llm_model=None):
        """
        Insert a new character or update an existing character record identified by Actor_id.
    ...
    def get_character(self, Actor_id):
        ...
        Returns:
            dict: A dictionary containing character details if found, otherwise None.
        """
    ...
    def save_story(self, narration_text, character_texts, narrator_audio_path=None):
        """
        Saves a story entry for the narrator and corresponding entries for each character.
    ...
    def get_story_history(self):
        """
        Retrieve the complete story log history as a list of entries ordered by timestamp.
    ...
    def save_training_data(self, dataset, Actor_id):
        ...
        Parameters:
            dataset (dict): Dictionary containing 'input' and 'output' text for training.
            Actor_id (str): Unique identifier for the actor associated with the training data.
        """
    ...
    def get_training_data_for_Actor(self, Actor_id):
        """
        Retrieve all training data entries associated with the specified Actor.
    ...
    def save_client_token(self, Actor_id, token):
        """
        Save or update a client token for the specified Actor_id, setting its status to 'Registered' and updating the last seen timestamp.
    ...
    def get_client_token_details(self, Actor_id):
        """
        Retrieve the token details and connection status for a client by Actor_id.
    ...
    def register_client(self, Actor_id, ip_address, client_port):
        """
        Register or update a client by setting its IP address, port, last seen timestamp, and status to 'Online_Heartbeat'.
    ...
    def update_client_status(self, Actor_id, new_status, last_seen_iso=None):
        """
        Update the status and last seen timestamp for a client token.
    ...
    def get_clients_for_story_progression(self):
        """
        Retrieve a list of clients that are currently responsive and eligible for story progression, excluding the primary actor.
        
        Returns:
            List[dict]: A list of dictionaries containing 'Actor_id', 'ip_address', and 'client_port' for each responsive client, excluding 'Actor1'. Returns an empty list if none are found.
        """
    ...
    def get_all_client_statuses(self):
        ...
        Returns:
            List[dict]: A list of dictionaries containing Actor_id, ip_address, client_port, last_seen, and status for each client.
        """
    ...
    def close(self):
        """
        Closes the current thread's SQLite database connection if it exists, releasing associated resources.
        """
    ...
    def __del__(self):
        """
        Destructor that ensures the database connection is closed when the Database instance is deleted.
        """
        self.close()
    ...
    def update_story_entry(self, entry_id, new_text=None, new_audio_path=None):
        ...
        Returns:
            bool: True if the entry was updated; False if no update fields were provided.
        """
        sets = []
        params = []
