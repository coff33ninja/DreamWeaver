...
class Database:
    ...
    def _execute_query(self, query, params=None, commit=False, fetchone=False, fetchall=False):
        ...
        Parameters:
            ...
            commit (bool, optional): If True, commits the transaction after execution.
            fetchone (bool, optional): If True, returns a single result row as a dictionary.
            fetchall (bool, optional): If True, returns all result rows as a list of dictionaries.
        
        Returns:
            The fetched row, list of rows, or cursor object depending on fetch flags. Returns None if a database error occurs.
        """
        conn = self._get_conn()
        try:
            cursor = conn.cursor()
    ...
    def _ensure_column(self, table_name, column_name, column_definition):
        ...
        Checks the table schema for the given column and adds it with the provided definition if it does not exist.
        """
        try:
    ...
    def _ensure_schema(self):
        """
        Create all required database tables and ensure necessary columns exist for schema compatibility.
        
        This method creates the `characters`, `story_log`, `training_data`, and `client_tokens` tables if they do not already exist, and adds any missing columns needed for new features or compatibility. It also ensures that default values and foreign key relationships are established as required for application functionality.
        """
        self._execute_query("""
    ...
    def save_character(self, name, personality, goals, backstory, tts, tts_model, reference_audio_filename, Actor_id, llm_model=None):
        """
        Insert a new character or update an existing one in the database using the provided Actor_id as a unique identifier.
        
        If a character with the given Actor_id already exists, its details are updated; otherwise, a new character record is created.
        """
        query = """
    ...
    def get_character(self, Actor_id):
        ...
        Returns:
            dict: A dictionary containing the character's details if found, including llm_model; otherwise, None.
        """
    ...
    def save_story(self, narration_text, character_texts, narrator_audio_path=None):
        """
        Save a story segment by recording a narrator entry and multiple character entries in the story log.
        
        Parameters:
            narration_text (str): The text spoken by the narrator.
            character_texts (dict): A mapping of character names to their spoken text.
    ...
    def get_story_history(self):
        ...
        Returns:
            List[dict]: A list of dictionaries, each representing a story log entry with keys: 'id', 'speaker', 'text', 'timestamp', and 'narrator_audio_path'.
        """
    ...
    def get_client_token_details(self, Actor_id):
        """
        Retrieve the token and connection details for a client identified by Actor_id.
        
        Returns:
            dict: A dictionary containing the token, status, IP address, client port, and last seen timestamp if the client exists; otherwise, None.
        """
    ...
    def update_client_status(self, Actor_id, new_status, last_seen_iso=None):
        ...
        Parameters:
            ...
            last_seen_iso (str, optional): ISO 8601 timestamp for the last time the client was seen. If not provided, the current UTC time is used.
        """
    ...
    def get_clients_for_story_progression(self):
        """
        Retrieve a list of clients that are currently responsive and eligible for story progression, excluding the special client 'Actor1'.
    ...
    def get_all_client_statuses(self):
        """
        Retrieve the status and connection details of all clients except 'Actor1', ordered by Actor ID.
        
        Returns:
            List[dict]: A list of dictionaries containing Actor ID, IP address, client port, last seen timestamp, and status for each client.
        """
    ...
    def close(self):
        """
        Closes the current thread's SQLite database connection and removes it from thread-local storage.
        """
    ...
    def __del__(self):
        """
        Ensures the database connection is closed when the Database object is deleted.
        """
        self.close()
