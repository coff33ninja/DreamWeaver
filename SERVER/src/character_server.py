...
class CharacterServer: # This is for Actor1, the server's own character
    def __init__(self, db):
        """
        Initialize the CharacterServer for the "Actor1" character, loading character data from the database or creating defaults if missing.
        
        If the character data is not found, a default character profile is created and saved to the database. LLM and TTS components are set to None for later asynchronous initialization.
        """
        self.db = db
    ...
    async def async_init(self):
        """
        Asynchronously initializes the LLM and TTS components for Actor1, preparing the character for text generation and speech synthesis.
        
        This method loads the LLM and TTS models in background threads to avoid blocking the event loop, and initializes the audio playback system. Should be called before generating responses or audio.
        """
    ...
    async def generate_response(self, narration: str, other_texts: dict) -> str:
        """
        Asynchronously generates a text response for the character based on narration and other character inputs.
        
        Combines the provided narration and other character texts into a prompt, generates a response using the character's language model, saves the interaction as training data, triggers asynchronous fine-tuning, and synthesizes audio output. Returns the generated text, or an error string if the character or LLM is not initialized.
        
        Parameters:
            narration (str): The main narration or context for the character's response.
            other_texts (dict): Additional text inputs from other characters, keyed by character name.
    ...
    async def output_audio(self, text: str): # speaker_wav removed, TTSManager instance holds it for XTTS
        """
        Asynchronously synthesizes speech from the provided text and plays the resulting audio for the character.
        
        If the character uses XTTSv2 and a reference audio file is available, it is used to guide voice synthesis. The synthesized audio is saved to a character-specific directory and played using pygame if the mixer is initialized. Logs warnings if TTS is not initialized, reference audio is missing, synthesis fails, or audio playback is not possible.
        """
            ...
            if pygame.mixer.get_init(): # Check if mixer is still initialized
                try:
                    ...
                    def play_sound():
                        ...
                        This function loads the specified audio file and plays it asynchronously. It does not block execution while the sound is playing.
                        """
...
if __name__ == '__main__':
    ...
    async def test_character_server():
        """
        Asynchronously tests the CharacterServer class by initializing it with a dummy database and verifying LLM and TTS setup.
        
        Runs an end-to-end test of response generation and audio output for the "Actor1" character, printing results to the console.
        """
...
Simulate saving training data for a specific actor by printing the data and actor ID.
"""
