...
class CharacterServer: # This is for Actor1, the server's own character
    ...
    async def async_init(self):
        """
        Asynchronously initializes the LLM and TTS engines for Actor1, preparing the character for text generation and speech synthesis.
        
        This method loads the LLM and TTS models in background threads to avoid blocking the event loop. It also initializes the pygame mixer for audio playback, handling any initialization errors.
        """
    ...
    async def generate_response(self, narration: str, other_texts: dict) -> str:
        """
        Asynchronously generates a character response using the LLM based on narration and other character texts, then synthesizes and plays the response as audio.
    ...
    async def output_audio(self, text: str): # speaker_wav removed, TTSManager instance holds it for XTTS
        ...
        If the character uses XTTSv2 and a reference audio file is available, it is used to guide synthesis. The generated audio is saved to a character-specific directory and played back using pygame if the mixer is initialized. Logs warnings if TTS is uninitialized, synthesis fails, reference audio is missing, or playback encounters errors.
        """
            ...
            if pygame.mixer.get_init(): # Check if mixer is still initialized
                try:
                    ...
                    def play_sound():
                        ...
                        This function initiates playback of the specified audio file. It does not block execution while the sound is playing.
                        """
...
if __name__ == '__main__':
    ...
    async def test_character_server():
        ...
        This function creates a mock database for Actor1, ensures necessary audio directories exist, initializes the CharacterServer asynchronously, and generates a sample response. It prints the results of initialization and response generation for verification.
        """
