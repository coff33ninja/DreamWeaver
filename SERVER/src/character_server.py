...
class CharacterServer:  # This is for Actor1, the server's own character
    def __init__(self, db):
        ...
        if not self.character:
            logger.warning(
                f"CharacterServer: {self.character_Actor_id} character data not found in DB. Creating and using defaults."
            )
            self.character = {
                "name": "Actor1_Default",
                ...
                "backstory": "embedded",
                "tts": "piper",
                "tts_model": "en_US-ryan-high",  # Default TTS
                "reference_audio_filename": None,
                "Actor_id": self.character_Actor_id,
                "llm_model": None,  # LLMEngine will use its default
            }
            try:
                self.db.save_character(**self.character)
                logger.info(
                    f"CharacterServer: Saved default character profile for {self.character_Actor_id} to DB."
                )
            except Exception as e:
                logger.error(
                    f"CharacterServer: Failed to save default character profile for {self.character_Actor_id} to DB: {e}",
                    exc_info=True,
                )
        else:
            logger.info(
                f"CharacterServer: Loaded character data for {self.character_Actor_id} from DB."
            )

        self.llm = None
        self.tts = None
        logger.info(
            f"CharacterServer for {self.character_Actor_id} initialized. LLM and TTS to be initialized asynchronously."
        )
    ...
    async def async_init(self):
        ...
        """
        logger.info(
            f"CharacterServer ({self.character_Actor_id}): Starting asynchronous initialization of LLM and TTS..."
        )
        ...
        try:
            ...
            logger.info(
                f"CharacterServer ({self.character_Actor_id}): Initializing LLMEngine with model '{llm_model_name}'..."
            )
            self.llm = await loop.run_in_executor(
                None, lambda: LLMEngine(model_name=llm_model_name, db=self.db)
            )
            if self.llm and self.llm.is_initialized:
                logger.info(
                    f"CharacterServer ({self.character_Actor_id}): LLMEngine initialized successfully."
                )
            else:
                logger.warning(
                    f"CharacterServer ({self.character_Actor_id}): LLMEngine failed to initialize or is_initialized is false."
                )
        except Exception as e_llm:
            logger.error(
                f"CharacterServer ({self.character_Actor_id}): Error initializing LLMEngine: {e_llm}",
                exc_info=True,
            )
            self.llm = None
        ...
        try:
            ...
            if tts_service == "xttsv2" and ref_audio_filename:
                speaker_wav = os.path.join(
                    REFERENCE_VOICES_AUDIO_PATH, ref_audio_filename
                )
                if not os.path.exists(speaker_wav):
                    logger.warning(
                        f"CharacterServer ({self.character_Actor_id}): Reference audio {speaker_wav} not found for XTTSv2. Will use default voice."
                    )
                    speaker_wav = None  # Fallback to default voice

            logger.info(
                f"CharacterServer ({self.character_Actor_id}): Initializing TTSManager with service '{tts_service}', model '{tts_model}'..."
            )
            ...
            if self.tts and self.tts.is_initialized:
                logger.info(
                    f"CharacterServer ({self.character_Actor_id}): TTSManager initialized successfully."
                )
            else:
                logger.warning(
                    f"CharacterServer ({self.character_Actor_id}): TTSManager failed to initialize or is_initialized is false."
                )
        except Exception as e_tts:
            logger.error(
                f"CharacterServer ({self.character_Actor_id}): Error initializing TTSManager: {e_tts}",
                exc_info=True,
            )
            self.tts = None
        ...
        if not pygame.mixer.get_init():
            try:
                pygame.mixer.init()
                logger.info(
                    f"CharacterServer ({self.character_Actor_id}): Pygame mixer initialized for audio playback. (Note: Unusual for server)"
                )
            except pygame.error as e_pygame:
                logger.warning(
                    f"CharacterServer ({self.character_Actor_id}): Pygame mixer could not be initialized: {e_pygame}. Audio playback will fail.",
                    exc_info=True,
                )
        logger.info(
            f"CharacterServer ({self.character_Actor_id}): Asynchronous initialization complete."
        )
    ...
    async def generate_response(self, narration: str, other_texts: dict) -> str:
        ...
        if not self.character:  # Should have been caught by __init__
            logger.error(
                f"CharacterServer ({char_name}): Character not loaded. Cannot generate response."
            )
            return ""
        if not self.llm or not self.llm.is_initialized:
            logger.error(
                f"CharacterServer ({char_name}): LLM not initialized. Cannot generate response for narration: '{narration[:50]}...'."
            )
        ...
        prompt_parts.append(
            f"Character: {char_name} responds as {self.character.get('personality', 'default')}:"
        )
        prompt = "\n".join(prompt_parts)

        logger.info(
            f"CharacterServer ({char_name}): Generating LLM response for prompt: '{prompt[:100]}...'"
        )
        text = await self.llm.generate(prompt, max_new_tokens=120)

        if (
            text
            and not text.startswith(f"[{char_name}_LLM_ERROR")
            and not text.startswith("[LLM_ERROR")
        ):  # Check specific and generic LLM errors
            logger.info(
                f"CharacterServer ({char_name}): LLM generated response: '{text[:100]}...'"
            )
            try:
                self.db.save_training_data(
                    {"input": prompt, "output": text}, self.character_Actor_id
                )
                logger.debug(f"CharacterServer ({char_name}): Saved training data.")
            except Exception as e_db_train:
                logger.error(
                    f"CharacterServer ({char_name}): Error saving training data: {e_db_train}",
                    exc_info=True,
                )

            if self.llm:  # LLM is confirmed initialized above
                logger.debug(
                    f"CharacterServer ({char_name}): Initiating fine-tuning (placeholder in LLMEngine)."
                )
                await self.llm.fine_tune(
                    {"input": prompt, "output": text}, self.character_Actor_id
                )

            logger.debug(
                f"CharacterServer ({char_name}): Synthesizing audio output for response."
            )
            await self.output_audio(text)
        else:
            logger.error(
                f"CharacterServer ({char_name}): LLM generation failed or returned error. Response: '{text}'"
            )
            # text might already be an error string from LLMEngine like "[LLM_ERROR: GENERATION_FAILED]"
            if not text:
                text = f"[{char_name}_LLM_ERROR:EMPTY_RESPONSE]"
    ...
    async def output_audio(self, text: str):
        ...
        if not self.tts or not self.tts.is_initialized:
            logger.warning(
                f"CharacterServer ({char_name}): TTS not initialized. Cannot output audio for text: '{text[:50]}...'."
            )
            return
        if not text:
            logger.warning(
                f"CharacterServer ({char_name}): Text is empty. No audio to output."
            )
            return
        ...
        try:
            ...
            logger.error(
                f"CharacterServer ({char_name}): Could not create audio directory {character_audio_dir}: {e_mkdir}",
                exc_info=True,
            )
            return
        ...
        if self.character.get("tts") == "xttsv2" and self.character.get(
            "reference_audio_filename"
        ):
            ref_path = os.path.join(
                REFERENCE_VOICES_AUDIO_PATH, self.character["reference_audio_filename"]
            )
            if os.path.exists(ref_path):
                speaker_wav_to_use = ref_path
                logger.debug(
                    f"CharacterServer ({char_name}): Using reference audio {ref_path} for XTTSv2."
                )
            else:
                logger.warning(
                    f"CharacterServer ({char_name}): Reference audio {ref_path} not found for XTTSv2. TTS will use default voice."
                )

        logger.info(
            f"CharacterServer ({char_name}): Attempting to synthesize audio to {final_audio_path} for text: '{text[:50]}...'."
        )
        success = await self.tts.synthesize(
            text, final_audio_path, speaker_wav_for_synthesis=speaker_wav_to_use
        )

        if success and os.path.exists(final_audio_path):
            logger.info(
                f"CharacterServer ({char_name}): Audio successfully synthesized to {final_audio_path}"
            )
            # JULES: Pygame audio playback.
            # As noted in async_init, server-side audio playback via pygame is unusual.
            # This might be for local testing. For remote users (narrator),
            # audio should ideally be streamed to the UI.
            if pygame.mixer.get_init():
                try:
                    ...
                    def play_sound_blocking():
                        ...
                        ):  # Ensure sound plays out in the thread
                            pygame.time.Clock().tick(10)

                    logger.debug(
                        f"CharacterServer ({char_name}): Playing audio {final_audio_path} (via Pygame)"
                    )
                    await asyncio.to_thread(play_sound_blocking)
                    logger.info(
                        f"CharacterServer ({char_name}): Finished playing audio {final_audio_path} (via Pygame)"
                    )
                except Exception as e_play:
                    logger.error(
                        f"CharacterServer ({char_name}): Error playing audio {final_audio_path} with Pygame: {e_play}",
                        exc_info=True,
                    )
            else:
                logger.warning(
                    f"CharacterServer ({char_name}): Pygame mixer not initialized. Cannot play audio {final_audio_path}."
                )
        else:
            logger.error(
                f"CharacterServer ({char_name}): Audio synthesis failed or output file not found/empty at {final_audio_path}."
            )
    ...
    async def test_character_server():
        ...
        class DummyDB:
            ...
            def save_character(self, **kwargs):
                ...
                logger.debug(
                    f"DummyDB: save_character called for {actor_id} with data: {kwargs}"
                )
                if actor_id:
        ...
        logger.info(
            "--- Test Case 1: Initializing CharacterServer with empty DB (should create default Actor1) ---"
        )
        ...
        actor1_config = {
            "name": "ConfiguredActor1",
            "personality": "configured_tester",
            "tts": "gtts",
            "language": "en",
            "tts_model": "en",  # gTTS uses lang code as model effectively
            "reference_audio_filename": None,
            "Actor_id": "Actor1",
            "llm_model": "some_test_llm",
        }
        dummy_db_instance_filled.save_character(**actor1_config)

        logger.info(
            "--- Test Case 2: Initializing CharacterServer with pre-configured Actor1 ---"
        )
        ...
        if cs.llm and cs.llm.is_initialized and cs.tts and cs.tts.is_initialized:
            logger.info(
                "CharacterServer (Actor1) initialized with LLM and TTS for Test Case 2."
            )
            ...
            logger.error(
                "CharacterServer (Actor1) LLM or TTS failed to initialize for Test Case 2."
            )
            if cs.llm:
