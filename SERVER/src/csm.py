...
class CSM:
    ...
    async def process_story(self, audio_filepath: str, chaos_level: float):
        ...
        """
        logger.info(
            f"CSM: Starting to process story for audio_filepath: {audio_filepath}, chaos_level: {chaos_level}"
        )
        ...
        if not narration_text:
            logger.warning(
                "CSM: Narration text empty after processing. Skipping further story processing for this turn."
            )
            return "", {}
        ...
        server_character_details = await asyncio.to_thread(
            self.db.get_character, "Actor1"
        )  # DB call in thread
        if server_character_details:
            logger.debug(
                f"CSM: Getting response from server character: {server_character_details.get('name', 'Actor1')}"
            )
            # CharacterServer.generate_response is now async
            server_response_text = await self.character_server.generate_response(
                narration_text, {}
            )
            if server_response_text:
                character_texts[server_character_details.get("name", "Actor1")] = (
                    server_response_text
                )
                logger.debug(
                    f"CSM: Server character {server_character_details.get('name', 'Actor1')} responded."
                )
            else:
                logger.debug(
                    f"CSM: Server character {server_character_details.get('name', 'Actor1')} provided no response."
                )
        else:
        ...
        responsive_clients = await asyncio.to_thread(
            self.client_manager.get_clients_for_story_progression
        )
        logger.info(
            f"CSM: Found {len(responsive_clients)} responsive clients for story progression."
        )
        ...
        if responsive_clients:
            ...
            client_actor_ids = [
                client["Actor_id"]
                for client in responsive_clients
                if client.get("Actor_id")
            ]
            all_character_details = await asyncio.to_thread(
                self.db.get_characters_by_ids, client_actor_ids
            )
            ...
            for client_info in responsive_clients:
                ...
                if not all([client_actor_id, client_ip, client_port]):
                    logger.error(
                        f"CSM: Skipping client due to missing info from DB record: {client_info}"
                    )
                    continue
                ...
                if not client_character_details:
                    logger.warning(
                        f"CSM: No character details found for responsive client {client_actor_id}. Skipping."
                    )
                    continue
            ...
            if client_tasks:
                logger.debug(
                    f"CSM: Gathering responses from {len(client_tasks)} clients concurrently..."
                )
                ...
                task_results = await asyncio.gather(
                    *(task for _, task in client_tasks), return_exceptions=True
                )
                ...
                for (char_details, _), result in zip(client_tasks, task_results):
                    ...
                    if isinstance(result, Exception):
                        logger.error(
                            f"CSM: Error processing/gathering response from client {char_name}: {result}",
                        ...
                        logger.debug(
                            f"CSM: Client {char_name} provided no/empty response."
                        )
        ...
        if chaos_level > 0 and self.chaos_engine.random_factor() < (chaos_level / 10.0):
            ...
            narration_text, character_texts = self.chaos_engine.apply_chaos(
                narration_text, character_texts
            )
            logger.debug(
                f"CSM: Chaos applied. Narration: '{narration_text[:50]}...', Char texts: { {k: v[:50] for k,v in character_texts.items()} }"
            )
        ...
        if narration_text or character_texts:
            ...
            await asyncio.to_thread(
                ...
                narration_text,
                character_texts,
                narrator_audio_path_db,
            )
            logger.info("CSM: Story turn processed and saved.")
        else:
            logger.info(
                "CSM: Nothing to save for this story turn (empty narration and character texts)."
            )
    ...
    def update_last_narration_text(self, new_text):
        ...
        if narrator_entries := [
            ...
            logger.info(
                f"CSM: Updated last narrator transcription in DB (id={last_entry['id']}) with new text: {new_text[:100]}..."
            )
            return True
