...
class CSM:
    def __init__(self):
        """
        Initialize the CSM system and its core components.
        
        Creates and configures the database, narrator, character server, client manager, hardware interface, and chaos engine. Starts periodic health checks for connected clients.
        """
    ...
    async def process_story(self, audio_filepath: str, chaos_level: float):
        """
        Asynchronously processes a story turn by generating narration, collecting responses from the server character and all responsive clients, applying chaos effects, updating hardware, and saving the results to the database.
    ...
    def update_last_narration_text(self, new_text):
        ...
        Returns:
            bool: True if the last narrator entry was updated, False if no such entry exists.
        """
