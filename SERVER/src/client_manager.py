...
class ClientManager:
    def __init__(self, db: Database):
        ...
        Attempts to initialize the pygame mixer for audio playback, issuing a warning if initialization fails. Sets up threading constructs for managing periodic client health checks.
        """
        self.db = db
    ...
    def generate_token(self, Actor_id: str) -> str:
        """
        Generate and store a unique authentication token for the specified Actor.
        
        If the Actor does not exist in the database and the Actor_id is "Actor1", creates a default internal server character before generating the token. Stores the generated token in the database and returns it.
    ...
    def get_clients_for_story_progression(self):
        ...
        Returns:
            List of client records used to determine story progression.
        """
        return self.db.get_clients_for_story_progression()

    def validate_token(self, Actor_id: str, token: str) -> bool:
        """
        Check if the provided token matches the stored token for the given Actor and that the client is not deactivated.
        
        Returns:
            bool: True if the token is valid and the client is active; False otherwise.
        """
    ...
    def _perform_single_health_check_blocking(self, client_info: dict):
        """
        Performs a synchronous health check on a client and updates its status in the database.
        
        Checks the client's `/health` endpoint using its IP address and port. Updates the client's status based on the response or encountered errors.
        """
    ...
    def _periodic_health_check_loop(self):
        """
        Continuously monitors the health of all clients, updating their status based on responsiveness and heartbeat freshness.
        
        This loop runs in a background thread, periodically checking each client's status and performing health checks as needed. Clients with stale heartbeats are marked as offline. The loop exits when the stop event is set.
        """
    ...
    def start_periodic_health_checks(self):
        """
        Starts the background thread that performs periodic health checks on all clients if it is not already running.
        """
    ...
    async def send_to_client(self, client_Actor_id: str, client_ip: str, client_port: int, narration: str, character_texts: dict) -> str:
        """
        Send narration and character texts to a client, handling retries and audio playback.
        
        Attempts to deliver narration and character-specific texts to a client via HTTP POST, using the client's token for authentication. Retries the request on failure with exponential backoff. If the response includes audio data and audio playback is available, decodes and plays the audio. Updates the client's status in the database based on the outcome.
