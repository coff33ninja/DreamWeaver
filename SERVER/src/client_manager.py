...
class ClientManager:
    def __init__(self, db: Database):
        ...
        Attempts to initialize the pygame mixer for audio playback, issuing a warning if initialization fails. Prepares threading constructs for periodic client health checks.
        """
        self.db = db
    ...
    def generate_token(self, Actor_id: str) -> str:
        ...
        If the character does not exist and the Actor ID is "Actor1", creates a default internal server character before generating the token. The generated token is saved in the database and returned.
    ...
    def get_clients_for_story_progression(self):
        """
        Retrieve clients that are eligible for story progression from the database.
        
        Returns:
            list: A list of clients suitable for advancing the story.
        """
    ...
    def _periodic_health_check_loop(self):
        ...
        This loop runs in a background thread, checking each client's status at regular intervals. It performs health checks for clients with problematic or heartbeat statuses, and marks clients as offline if their last heartbeat is stale. The loop exits when signaled to stop.
        """
    ...
    def start_periodic_health_checks(self):
        """
        Starts the background thread that periodically checks the health status of all clients if it is not already running.
        """
    ...
    async def send_to_client(self, client_Actor_id: str, client_ip: str, client_port: int, narration: str, character_texts: dict) -> str:
        """
        Asynchronously sends narration and character texts to a client via HTTP POST, handling retries, status updates, and optional audio playback.
        
        Attempts to deliver narration and character-specific texts to the specified client endpoint, using the client's token for authentication. On success, updates the client's status and plays any returned audio data if available. Handles connection errors, timeouts, and request failures with retries and exponential backoff, updating client status accordingly.
