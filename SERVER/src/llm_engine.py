...
class LLMEngine:
    ...
    def _load_model(self):
        """
        Load the tokenizer and model for the specified model name, applying 4-bit quantization and LoRA adapter integration if available.

        If CUDA is available, loads the model with BitsAndBytes 4-bit quantization and prepares it for k-bit training. Attempts to load existing LoRA adapters for "Actor1"; if not found, initializes a new LoRA configuration for future fine-tuning. Sets the model to evaluation mode and updates the initialization status. On failure, resets model state and initialization flag.
        """
    ...
    async def fine_tune(self, new_data_point: dict, Actor_id: str):
        """
        Asynchronously fine-tunes the language model for "Actor1" using all available training data and a new data point.

        This method fetches all training data for "Actor1" from the database, appends the new data point if not already present, and performs LoRA-based fine-tuning in a background thread. Fine-tuning is only performed for "Actor1" and requires the engine to be initialized with a database interface. After training, LoRA adapters are saved to the configured adapter path. The model is returned to evaluation mode upon completion or error.
        """
        if not self.is_initialized or not self.model or not self.tokenizer:
        ...
        def _blocking_fine_tune():
            """
            Performs blocking fine-tuning of the loaded language model on all available training data for Actor1.
            ...
            def tokenize_function(examples):
                """
                Tokenizes input text examples with truncation and padding to the model's maximum length.
