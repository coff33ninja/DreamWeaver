...
class ChaosEngine:
    ...
    def _parse_neo_data(self, neo_data: dict) -> str:
        ...
        miss_distance_km = float(neo_data.get("close_approach_data", [{}])[0].get("miss_distance", {}).get("kilometers", "unknown"))
        velocity_km_s = float(neo_data.get("close_approach_data", [{}])[0].get("relative_velocity", {}).get("kilometers_per_second", "unknown"))

        return (f"A cosmic event unfolds! {event_name} makes a close approach, "
    ...
    def _get_nasa_neo_event(self) -> str | None:
        ...
        try:
            ...
            if near_earth_objects := data.get("near_earth_objects", {}):
                if all_neos_today := [neo for date_key in near_earth_objects for neo in near_earth_objects[date_key]]:
    ...
    def apply_chaos(self, narration, character_texts):
        ...
        if self.nasa_api_key != "DEMO_KEY" and random.random() < 0.1:
            if event_description := self._get_nasa_neo_event():
                ...
                logger.info(f"ChaosEngine: Major chaos event triggered: {event_description}")
                return narration, character_texts # Return early if a major event happened
