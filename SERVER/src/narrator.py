...
class Narrator:
            ...
            while retry_count < max_retries:
                try:
                    ...
                    if urls:
                        for url in urls:
                            logger.info(
                                f"[Narrator] Opening required page for diarization setup: {url}"
                            )
                            webbrowser.open(url)
                        logger.info(
                            "[Narrator] Please complete any required actions in your browser (e.g., accept TOS, login, or generate a token).\n"
                        )
                    else:
                        logger.info(
    ...
    async def process_narration(self, audio_filepath: str) -> dict:
        ...
        try:
            ...
            logger.error(
                f"Narrator: Failed to save audio copy from {audio_filepath} to {dest_path}: {e}",
                exc_info=True,
            )
        ...
        try:
            logger.info(
                f"Narrator: Transcribing audio file: {dest_path} (original: {audio_filepath})..."
            )
            transcription_result = await asyncio.to_thread(
                self.stt_model.transcribe, dest_path, fp16=False
            )
            text_field = transcription_result.get("text", "")
            if isinstance(
                text_field, list
            ):  # Should not happen with standard whisper output
                transcribed_text = " ".join(str(s) for s in text_field).strip()
                logger.warning(
                    f"Narrator: Transcription result text was a list, joined: '{transcribed_text[:50]}...'"
                )
            else:
                transcribed_text = str(text_field).strip()
            logger.info(
                f"Narrator: Transcription complete. Text: '{transcribed_text[:50]}...'"
            )
            ...
            if self.diarization_pipeline and transcribed_text:
                try:
                    ...
                    diarization_output = await asyncio.to_thread(
                        self.diarization_pipeline, dest_path
                    )
                    speaker_durations = {}
                    for turn, _, label in diarization_output.itertracks(
                        yield_label=True
                    ):
                        duration = turn.end - turn.start
                        speaker_durations[label] = (
                    ...
                    logger.error(
                        f"Narrator: Error during diarization for {dest_path}: {e_diar}. Using default speaker.",
                        exc_info=True,
                    )
            ...
            logger.error(
                f"Narrator: Error processing narration for {dest_path}: {e_transcribe}",
                exc_info=True,
            )
            return {
                "text": "",
    ...
    async def test_narrator():
        ...
        if not os.path.exists(dummy_audio):
            try:
                ...
                with wave.open(dummy_audio, "w") as wf:
                    ...
                    for i in range(num_samples):
                        value = int(
                            32767.0
                            * math.cos(
                                frequency * math.pi * float(i) / float(sample_rate)
                            )
                        )
                ...
                logger.error(
                    f"Could not create dummy audio file: {e}. Please create '{dummy_audio}' manually to test.",
                    exc_info=True,
                )
                return
        ...
        if os.path.exists(dummy_audio):
            ...
            logger.warning(
                f"Dummy audio file '{dummy_audio}' not found. Skipping STT test."
            )
