...
class TTSManager:
    def __init__(self, tts_service_name: str, model_name: Optional[str] = None, speaker_wav_path: Optional[str] = None, language: Optional[str] = 'en'):
        ...
        Parameters:
            ...
            model_name (Optional[str]): The identifier or path for the TTS model, if applicable.
        ...
        Sets up environment variables, ensures required directories exist, and synchronously initializes the selected TTS backend.
        """
    ...
    def _initialize_service_blocking(self):
        ...
        Sets up the appropriate TTS synthesis method or model instance based on the selected service and model name. Marks the manager as initialized if successful; otherwise, logs errors or warnings for missing dependencies, unsupported services, or unavailable models.
        """
    ...
    def _gtts_synthesize_blocking(self, text: str, output_file_path: str, lang: str):
        """
        Synthesize speech from text using the gTTS backend and save the result to a file.
        
        Parameters:
            text (str): The text to be converted to speech.
            output_file_path (str): The path where the synthesized audio file will be saved.
    ...
    async def synthesize(self, text: str, output_filename_no_path: str, speaker_wav_for_synthesis: Optional[str] = None) -> str | None:
        ...
        Parameters:
            ...
            speaker_wav_for_synthesis (Optional[str]): Optional path to a speaker WAV file for voice cloning (used with certain TTS backends).
        
        Returns:
            str | None: The full path to the generated audio file on success, or None if synthesis fails or the manager is not initialized.
        """
    ...
    def list_services():
        ...
        Returns:
            services (list): List of supported TTS service names.
        """
        services = []
        if gtts:
    ...
    def get_available_models(service_name: str):
        ...
        Returns:
            list: A list of model identifiers or descriptions available for the given service. Returns an empty list if the service is unsupported.
        """
...
if __name__ == "__main__":
    async def test_async_tts_manager():
        ...
        This function initializes TTSManager instances for each supported service (gTTS and XTTSv2), synthesizes sample text asynchronously, and prints the paths to the generated audio files. Output files are saved in the configured temporary audio directory.
        """
