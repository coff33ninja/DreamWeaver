...
class TTSManager:
    def __init__(self, tts_service_name: str, model_name: Optional[str] = None, speaker_wav_path: Optional[str] = None, language: Optional[str] = 'en'):
        ...
        Parameters:
            tts_service_name (str): The name of the TTS backend to use (e.g., "gtts", "xttsv2").
            model_name (str, optional): The model identifier or path for services that require it.
            speaker_wav_path (str, optional): Path to a speaker WAV file for voice cloning (used by some services).
            language (str, optional): Language code for synthesis (default is "en").
        
        Initializes the TTS backend synchronously, prepares model directories, and sets up environment variables as needed.
        """
    ...
    def _initialize_service_blocking(self):
        ...
        Selects and configures the appropriate TTS engine based on the specified service name and model. For `gtts`, sets up the internal synthesis method if the library is available. For `xttsv2`, loads the specified model using Coqui TTS if available, assigning it to GPU if possible. Logs errors or warnings if dependencies or models are missing.
        """
    ...
    def _gtts_synthesize_blocking(self, text: str, output_file_path: str, lang: str):
        """
        Synchronously synthesizes speech from text to an audio file using the gTTS backend.
    ...
    def _xttsv2_synthesize_blocking(self, text: str, output_file_path: str, speaker_wav: Optional[str] = None, lang: str = "en"):
        """
        Synthesize speech from text to an audio file using the XTTSv2 backend.
    ...
    async def synthesize(self, text: str, output_filename_no_path: str, speaker_wav_for_synthesis: Optional[str] = None) -> str | None:
        ...
        Parameters:
            text (str): The text to be converted to speech.
            output_filename_no_path (str): The output filename (without path) for the synthesized audio file.
    ...
    def _get_or_download_model_blocking(self, service_name: str, model_identifier: str):
        ...
        For the "xttsv2" service, returns the provided model identifier. Returns None for unsupported services.
        """
    ...
    def list_services():
        ...
        Returns:
            services (list of str): Names of supported TTS services currently available in the environment.
        """
        services = []
        if gtts:
