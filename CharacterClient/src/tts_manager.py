...
try:
    ...
    gtts = None
    logger.info("Client TTSManager: gtts library not found. gTTS service will not be available.")
try:
    from TTS.api import TTS as CoquiTTS
except ImportError:
    CoquiTTS = None
    logger.info("Client TTSManager: Coqui TTS library not found. XTTSv2 service will not be available.")
...
class TTSManager:
    def __init__(self, tts_service_name: str, model_name: Optional[str] = None, speaker_wav_path: Optional[str] = None, language: Optional[str] = 'en'):
        ...
        try:
            ...
            logger.error(f"Client TTSManager: Could not create tts_models directory {os.path.join(CLIENT_TTS_MODELS_PATH, 'tts_models')}: {e}", exc_info=True)
            # Depending on severity, might want to set self.is_initialized = False or raise

        logger.info(f"Client TTSManager: Initializing for service '{self.service_name}', model '{self.model_name}', lang '{self.language}', speaker_wav: '{self.speaker_wav_path}'")
    ...
    def _initialize_service_blocking(self):
        ...
        if self.service_name == "gtts":
            if gtts:
                ...
                logger.error("Client TTSManager: Error - gTTS library not found. gTTS service unavailable.")
            return

        if not self.model_name: # This check is more relevant for CoquiTTS
            logger.warning(f"Client TTSManager: No model name specified for service '{self.service_name}'. Initialization may fail or use defaults.")
        ...
        if not model_path_or_name and self.service_name == "xttsv2": # If xttsv2 and model_name was expected but not resolved
            logger.error(f"Client TTSManager: Could not determine model path or name for '{self.model_name}' for service '{self.service_name}'. Cannot initialize.")
            return

        if self.service_name == "xttsv2":
            if CoquiTTS:
                try:
                    logger.info(f"Client TTSManager: Initializing Coqui XTTSv2 with model: {model_path_or_name or self.model_name}")
                    self.tts_instance = CoquiTTS(model_name=model_path_or_name or self.model_name, progress_bar=True) # Use self.model_name if model_path_or_name is None (e.g. not used by _get_or_download)
                    device = "cuda" if torch.cuda.is_available() else "cpu"
                    self.tts_instance.to(device)
                    self.is_initialized = True
                    logger.info(f"Client TTSManager: Coqui XTTSv2 initialized with {model_path_or_name or self.model_name}. Device: {device}")
                except Exception as e:
                    logger.error(f"Client TTSManager: Error initializing Coqui XTTSv2 model {model_path_or_name or self.model_name}: {e}", exc_info=True)
            else:
                logger.error("Client TTSManager: Error - Coqui TTS library not found. XTTSv2 service unavailable.")
        elif self.service_name != "gtts": # gtts is handled above
             logger.error(f"Client TTSManager: Unsupported TTS service '{self.service_name}'.")
    ...
    def _gtts_synthesize_blocking(self, text: str, output_file_path: str, lang: str):
        ...
        if gtts and hasattr(gtts, 'gTTS'):
            logger.debug(f"Client TTSManager (gTTS): Synthesizing text '{text[:30]}...' to {output_file_path} in lang '{lang}'")
            gtts.gTTS(text=text, lang=lang).save(output_file_path)
        else:
            logger.error("Client TTSManager: gTTS library not available or gTTS class missing. Cannot synthesize with gTTS.")
    ...
    def _xttsv2_synthesize_blocking(self, text: str, output_file_path: str, speaker_wav: Optional[str] = None, lang: str = "en"):
        ...
        if self.tts_instance is None or not hasattr(self.tts_instance, 'tts_to_file') or not callable(getattr(self.tts_instance, 'tts_to_file')):
            logger.error("Client TTSManager (XTTSv2): XTTSv2 instance is not available or invalid. Cannot synthesize.")
            return

        lang_to_use = lang or "en"
        logger.debug(f"Client TTSManager (XTTSv2): Preparing to synthesize text '{text[:30]}...' to {output_file_path}. Speaker: '{speaker_wav}', Lang: '{lang_to_use}'.")
        ...
        if speaker_to_use and isinstance(speaker_to_use, str) and os.path.exists(speaker_to_use):
            ...
            logger.debug(f"Client TTSManager (XTTSv2): Synthesized with speaker wav {speaker_to_use}.")
        else:
            if speaker_to_use: # Only log warning if a speaker_wav was intended but not found/valid
                logger.warning(f"Client TTSManager (XTTSv2): speaker_wav '{speaker_to_use}' not found or invalid. Using default voice for lang {lang_to_use}.")
            else: # Log if no speaker_wav was provided at all and default is used
                logger.debug(f"Client TTSManager (XTTSv2): Synthesizing with default voice for lang {lang_to_use}.")
    ...
    async def synthesize(self, text: str, output_filename_no_path: str, speaker_wav_for_synthesis: Optional[str] = None) -> str | None:
        ...
        if not self.is_initialized or not self.tts_instance:
            logger.error(f"Client TTSManager ({self.service_name}): Not initialized. Cannot synthesize text: '{text[:30]}...'.")
            return None
        ...
        try:
            os.makedirs(os.path.dirname(output_full_path), exist_ok=True)
             ...
             logger.error(f"Client TTSManager: Could not create directory for temporary audio file {output_full_path}: {e_mkdir}", exc_info=True)
             return None

        logger.info(f"Client TTSManager ({self.service_name}): Synthesizing text '{text[:30]}...' to {output_full_path}")
        try:
            if self.service_name == "gtts":
                ...
                logger.error(f"Client TTSManager: No async synthesis method for unsupported service '{self.service_name}'.")
                return None # Or raise ValueError

            if os.path.exists(output_full_path) and os.path.getsize(output_full_path) > 0:
                logger.info(f"Client TTSManager: Successfully synthesized audio to {output_full_path}")
                return output_full_path
            else:
                logger.error(f"Client TTSManager: Synthesis completed but output file {output_full_path} is missing or empty.")
                return None
        except Exception as e:
            logger.error(f"Client TTSManager: Error during async TTS synthesis with {self.service_name} for '{text[:30]}...': {e}", exc_info=True)
            if os.path.exists(output_full_path): # Attempt to clean up failed/partial file
                try:
                    os.remove(output_full_path)
                    logger.info(f"Client TTSManager: Removed partial/failed output file {output_full_path}")
                except OSError as e_remove:
                    logger.error(f"Client TTSManager: Error removing partial/failed output file {output_full_path}: {e_remove}", exc_info=True)
            return None
    ...
    def _get_or_download_model_blocking(self, service_name: str, model_identifier: str):
        ...
        try:
            ...
            logger.error(f"Client TTSManager: Could not create model directory {target_dir_base} for service {service_name}: {e}", exc_info=True)
        ...
        if service_name == "xttsv2":
            logger.debug(f"Client TTSManager: Model identifier for XTTSv2 is '{model_identifier}'. Coqui will handle download if needed to TTS_HOME={os.getenv('TTS_HOME')}")
            return model_identifier
        # For gTTS or other services not requiring explicit model file download via this manager
        logger.debug(f"Client TTSManager: No specific model download logic for service '{service_name}' in this manager. Identifier: '{model_identifier}'")
    ...
    def get_available_models(service_name: str):
        ...
        Returns:
            list[str]: A list of available model identifiers or descriptions for the given service. Returns an empty list if the service is unsupported.
        """
...
if __name__ == "__main__":
    async def test_async_tts_manager():
        ...
        try:
            ...
            logger.error(f"Could not create test output directory {test_output_dir}: {e}. Aborting test.", exc_info=True)
            return
        ...
        if "gtts" in TTSManager.list_services():
            ...
            if tts_g.is_initialized:
                ...
                logger.warning("gTTS (async test) was not initialized, skipping synthesis test.")
        ...
        if "xttsv2" in TTSManager.list_services():
            ...
            if tts_x.is_initialized:
                ...
                if out_x_path:
                    logger.info(f"XTTSv2 async (default voice) test audio saved to {out_x_path}")
                else:
    ...
    if not logging.getLogger(LOGGER_NAME).hasHandlers(): # Check if logger is already configured
