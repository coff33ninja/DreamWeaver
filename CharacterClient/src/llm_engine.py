...
try:
    ...
    raise ImportError(
        "The 'peft' package is required for LoRA/adapter support. Please install it with 'pip install peft'."
    ) from e
...
class LLMEngine:
    def __init__(self, model_name: str = "", Actor_id: str = "default_client_Actor"):
        ...
        self.adapters_path = os.path.join(
            CLIENT_LLM_MODELS_PATH, "adapters", self.Actor_id, sane_model_name_for_path
        )
        ...
        logger.info(
            f"Client LLMEngine: Initializing for model '{self.model_name}' for Actor_ID '{self.Actor_id}'. Base models path: {self.models_base_path}, Adapters path: {self.adapters_path}"
        )
    ...
    def _load_model_and_tokenizer(self):
        ...
        try:
            ...
            self.tokenizer = AutoTokenizer.from_pretrained(
                self.model_name, cache_dir=self.models_base_path, trust_remote_code=True
            )
            ...
            adapter_config_file = os.path.join(
                self.adapters_path, "adapter_config.json"
            )
            if os.path.exists(adapter_config_file) and self.model:
                try:
                    logger.info(
                        f"Client LLMEngine ({self.Actor_id}): Found adapter config. Attempting to load LoRA adapters from {self.adapters_path}."
                    )
                    self.model = PeftModel.from_pretrained(
                        self.model, self.adapters_path
                    )
                    logger.info(
                        f"Client LLMEngine ({self.Actor_id}): Successfully loaded LoRA adapters from {self.adapters_path}."
                    )
                except Exception as e:
                    logger.warning(
                        f"Client LLMEngine ({self.Actor_id}): Error loading LoRA adapters from {self.adapters_path}: {e}. Proceeding with base model.",
                        exc_info=True,
                    )
            ...
            if self.model:
                ...
                device_type = (
                    self.model.device.type
                    if hasattr(self.model, "device")
                    else "unknown"
                )
                logger.info(
                    f"Client LLMEngine for {self.Actor_id} (model: {self.model_name}) initialized successfully on device: {device_type}."
                )
            else:
                logger.error(
                    f"Client LLMEngine ({self.Actor_id}): Model object is None after loading attempt for '{self.model_name}'."
                )
        except Exception as e:
            logger.critical(
                f"Client LLMEngine ({self.Actor_id}): FATAL Error during model loading for '{self.model_name}': {e}",
                exc_info=True,
            )
            self.is_initialized = False
    ...
    async def generate(self, prompt: str, max_new_tokens: int = 150) -> str:
        ...
        if not self.is_initialized or not self.model or not self.tokenizer:
            logger.error(
                f"Client LLMEngine ({self.Actor_id}): Not initialized, cannot generate text for prompt: '{prompt[:50]}...'."
            )
            return "[LLM_ERROR:NOT_INITIALIZED]"

        logger.info(
            f"Client LLMEngine ({self.Actor_id}): Generating text for prompt: '{prompt[:100]}...'"
        )
        ...
        def _blocking_generate_task():
            ...
            max_prompt_len = (
                (self.tokenizer.model_max_length or 2048) - max_new_tokens - 10
            )
        ...
        try:
            ...
            logger.info(
                f"Client LLMEngine ({self.Actor_id}): Generated text: '{generated_text[:100]}...'"
            )
            return generated_text
        except Exception as e:
            logger.error(
                f"Client LLMEngine ({self.Actor_id}): Error during async text generation for prompt '{prompt[:50]}...': {e}",
                exc_info=True,
            )
    ...
    async def fine_tune_async(self, training_data: dict, Actor_id_override: str = ""):
        ...
        logger.info(
            f"Client LLMEngine ({current_Actor_id}): Async fine-tuning requested with data: {str(training_data)[:100]}..."
        )
        ...
        def _save_data_blocking():
            ...
            data_dir = os.path.join(
                CLIENT_LLM_MODELS_PATH, "training_data_local", current_Actor_id
            )
            ...
            try:
                ...
                logger.info(
                    f"Client LLMEngine ({current_Actor_id}): Saved training data sample to {data_file}"
                )
            except Exception as e:
                logger.error(
                    f"Client LLMEngine ({current_Actor_id}): Error saving local training data to {data_file}: {e}",
                    exc_info=True,
                )
        ...
        def _load_all_training_data():
            ...
            for fname in os.listdir(data_dir):
                if fname.endswith(".json"):
                    try:
                        with open(
                            os.path.join(data_dir, fname), "r", encoding="utf-8"
                        ) as f:
                            sample = json.load(f)
                            if (
                        ...
                        logger.error(
                            f"Client LLMEngine ({current_Actor_id}): Error loading training sample {fname}: {e}",
                            exc_info=True,
                        )
            logger.info(
                f"Client LLMEngine ({current_Actor_id}): Loaded {len(all_data)} training samples from {data_dir}."
            )
            return all_data
        ...
        def _train_blocking():
            ...
            if not self.model or not self.tokenizer:
                logger.error(
                    f"Client LLMEngine ({current_Actor_id}): Model or tokenizer not initialized, skipping fine-tuning."
                )
                return

            logger.info(
                f"Client LLMEngine ({current_Actor_id}): Loading all training data for fine-tuning..."
            )
            all_data = _load_all_training_data()
            if not all_data:
                logger.info(
                    f"Client LLMEngine ({current_Actor_id}): No training data found, skipping fine-tuning."
                )
                return
            ...
            try:
                ...
                trainer.save_model(
                    output_dir
                )  # This saves the full model if not PeftModel, or adapters if PeftModel
                logger.info(
                    f"Client LLMEngine ({current_Actor_id}): Fine-tuning complete. Model/adapters saved to {output_dir}"
                )
            except Exception as e_train:
                logger.error(
                    f"Client LLMEngine ({current_Actor_id}): Error during model training or saving: {e_train}",
                    exc_info=True,
                )

        await asyncio.to_thread(_train_blocking)
        logger.info(
            f"Client LLMEngine ({current_Actor_id}): Async fine-tuning process complete."
        )
    ...
    def fine_tune(self, training_data: dict, Actor_id_override: str = ""):
        ...
        current_Actor_id = Actor_id_override if Actor_id_override else self.Actor_id
        logger.info(
            f"Client LLMEngine ({current_Actor_id}): Fine-tuning requested (SYNC placeholder). Data: {str(training_data)[:100]}..."
        )
        data_dir = os.path.join(
            CLIENT_LLM_MODELS_PATH, "training_data_local", current_Actor_id
        )
        ...
        )  # Handled by ensure_client_directories or earlier save, but good practice
        timestamp = torch.randint(
            0, 1000000, (1,)
        ).item()  # Still using torch for this random int
        ...
        try:
            ...
            with open(data_file, "w", encoding="utf-8") as f:
                json.dump(training_data, f, ensure_ascii=False, indent=4)
            logger.info(
                f"Client LLMEngine ({current_Actor_id}): Saved SYNC training data (placeholder) to {data_file}"
            )
        except Exception as e:
            logger.error(
                f"Client LLMEngine ({current_Actor_id}): Error saving SYNC training data (placeholder) to {data_file}: {e}",
                exc_info=True,
            )
        logger.warning(
            "Client LLMEngine: Actual SYNC fine-tuning not implemented. Data saved only."
        )
        ...
        if self.model and isinstance(self.model, PeftModel):
            ...
            def _save_adapters_blocking():
                ...
                if self.model is not None:
                    logger.info(
                        f"Client LLMEngine ({self.Actor_id}): Saving adapters to {self.adapters_path}..."
                    )
                    self.model.save_pretrained(self.adapters_path)
                    logger.info(
                        f"Client LLMEngine ({self.Actor_id}): Adapters successfully saved to {self.adapters_path}."
                    )
                else:
                    logger.error(
                        f"Client LLMEngine ({self.Actor_id}): Cannot save adapters, model is None."
                    )
            ...
            try:
                ...
                logger.error(
                    f"Client LLMEngine ({self.Actor_id}): Error saving LoRA adapters (async) to {self.adapters_path}: {e}",
                    exc_info=True,
                )
        else:
            logger.info(
                f"Client LLMEngine ({self.Actor_id}): Model is not a PeftModel or not initialized. Skipping adapter save (async)."
            )
...
if __name__ == "__main__":
    ...
    ):  # Check if logger is already configured by main client setup
        logging.basicConfig(
            level=logging.DEBUG,
    ...
    async def test_async_llm_engine():
        ...
        if engine.is_initialized:
            ...
            await engine.fine_tune_async(
                {"input": "Async test input", "output": "Async test output"}
            )  # This will log
