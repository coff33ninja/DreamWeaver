...
class CharacterClient:
    def __init__(self, token: str, Actor_id: str, server_url: str, client_port: int):
        ...
        This constructor sets up the basic attributes for the client but does not perform any blocking operations or network requests. To complete initialization, including registration and resource loading, call the `async_init()` method after construction.
        
        Parameters:
            token (str): Authentication token for the character client.
            Actor_id (str): Unique identifier for the character actor.
            server_url (str): URL of the server to connect to.
    ...
    async def create(cls, token: str, Actor_id: str, server_url: str, client_port: int):
        ...
        Initializes the client with the provided token, actor ID, server URL, and client port, then performs all required asynchronous setup before returning the ready-to-use instance.
        
        Returns:
            CharacterClient: The fully initialized CharacterClient object.
        """
    ...
    async def async_init(self):
        ...
        """
        Asynchronously initializes the CharacterClient by performing registration, fetching character traits, downloading reference audio if needed, and initializing TTS and LLM components.
        
        This method offloads blocking operations to background threads to avoid blocking the event loop. If registration or trait fetching fails, it logs critical errors and applies default character settings.
        """
        def _register():
            return self._register_with_server_blocking()
        def _fetch():
            """
            Fetches the character's traits from the server using a blocking call.
            
            Returns:
                dict or None: The character traits as a dictionary if successful, or None if the fetch fails.
            """
        ...
        def _download():
            ...
            Returns:
                The result of the reference audio download operation, as provided by the blocking helper method.
            """
    ...
    def _download_reference_audio_blocking(self):
        ...
        If the character traits or reference audio filename are missing, the method exits without downloading. On download failure, the local reference audio path is set to None.
        """
        if not self.character:
    ...
    def _fetch_traits_blocking(self):
        ...
        """
        Fetches character traits from the server synchronously.
    ...
    async def send_heartbeat_async(self, max_retries=DEFAULT_MAX_RETRIES, base_delay=DEFAULT_BASE_DELAY_SECONDS) -> bool:
        """
        Asynchronously sends a heartbeat signal to the server with retry and exponential backoff logic.
        ...
        def _blocking_heartbeat_call():
            """
            Send a blocking HTTP POST request with the specified payload to the configured URL as a heartbeat signal.
    ...
    async def generate_response_async(self, narration: str, character_texts: dict) -> str:
        ...
        Parameters:
            narration (str): The narration or context provided by the narrator.
            character_texts (dict): Dialogue or statements from other characters, keyed by character name.
        
        Returns:
            str: The generated response text for the character. If the LLM is not ready, returns a status message indicating unavailability.
        """
        ...
        def _save_training_data_blocking():
            """
            Sends training data consisting of input prompt and generated output to the server for storage in a blocking manner.
            """
    ...
    async def synthesize_audio_async(self, text_to_synthesize: str) -> str | None:
        """
        Asynchronously generates speech audio from the provided text using the character's TTS engine.
        
        Parameters:
            text_to_synthesize (str): The text to convert to speech.
        
        Returns:
            str | None: The file path to the generated audio, or None if TTS is not initialized.
        """
...
async def handle_character_generation_request(data: dict, request: Request):
    """
    Handles character response generation and audio synthesis for a POST request.
    
    Validates the provided token and narration, generates a character response using the LLM, synthesizes corresponding speech audio, encodes the audio as base64, and returns both the generated text and audio data in the response.
...
async def health_check(request: Request):
    """
    Health check endpoint that reports the readiness status of the character client's LLM and TTS subsystems.
    
    Returns:
        dict: A JSON object containing the overall status ("ok" or "degraded"), the Actor ID, readiness flags for LLM and TTS, and a detail message if any subsystem is not ready.
    """
    client: CharacterClient = request.app.state.character_client_instance
    if not client:
...
async def _heartbeat_task_runner(client: CharacterClient):
    """
    Periodically sends heartbeat signals for the given character client at fixed intervals.
...
def start_heartbeat_task(client: CharacterClient):
    """
    Starts the heartbeat background task for the given character client if it is not already running.
...
def initialize_character_client(token: str, Actor_id: str, server_url: str, client_port: int):
    """
    Initializes the CharacterClient singleton instance and starts the heartbeat task if not already running.
    
    If the client is not yet initialized, this function creates the necessary directories, asynchronously constructs and initializes the CharacterClient, and schedules the heartbeat background task. Handles both running and non-running event loop scenarios to ensure proper async initialization. If the client is already initialized, no action is taken.
    """
    global _heartbeat_task_instance
