...
class CharacterClient:
    def __init__(self, token: str, Actor_id: str, server_url: str, client_port: int):
        """
        Initialize a CharacterClient instance with basic configuration.
        
        This constructor sets up the initial attributes for the character client, including authentication token, actor ID, server URL, and client port. It does not perform any blocking operations or network requests; further asynchronous initialization must be performed by calling `async_init()`.
        """
        self.token = token
    ...
    async def async_init(self):
        ...
        """
        Asynchronously initializes the CharacterClient by registering with the server, fetching character traits, downloading reference audio if needed, and initializing TTS and LLM subsystems.
        
        This method offloads blocking I/O operations to background threads and sets up the client for subsequent response generation and audio synthesis. If registration or trait fetching fails, default values are used to ensure continued operation.
        """
        def _register():
            return self._register_with_server_blocking()
        def _fetch():
            """
            Fetches the character traits from the server in a blocking manner.
        ...
        def _download():
            ...
            Returns:
                The result of the blocking reference audio download operation.
            """
    ...
    def _download_reference_audio_blocking(self):
        ...
        If the character traits or reference audio filename are missing, the method exits without downloading. On failure, the local reference audio path is set to None.
        """
        if not self.character:
    ...
    def _register_with_server_blocking(self, max_retries=DEFAULT_MAX_RETRIES, base_delay=DEFAULT_BASE_DELAY_SECONDS) -> bool:
        ...
        """
        Register the client with the server using a blocking HTTP POST request, retrying on failure with exponential backoff.
    ...
    async def send_heartbeat_async(self, max_retries=DEFAULT_MAX_RETRIES, base_delay=DEFAULT_BASE_DELAY_SECONDS) -> bool:
        ...
        def _blocking_heartbeat_call():
            """
            Send a blocking POST request to the server's heartbeat endpoint with the specified payload.
    ...
    async def generate_response_async(self, narration: str, character_texts: dict) -> str:
        ...
        If the LLM or character traits are not ready, returns a placeholder message indicating the LLM is unavailable. Otherwise, constructs a prompt from the provided narration and character texts, generates a response using the LLM, triggers an asynchronous fine-tuning placeholder, and saves the prompt-response pair to the server for training.
        
        Parameters:
            narration (str): The narration text to include in the prompt.
            character_texts (dict): A mapping of character names to their dialogue or context.
        
        Returns:
            str: The generated character response, or a placeholder message if the LLM is not ready.
        """
...
async def handle_character_generation_request(data: dict, request: Request):
    """
    Handles a character generation request by producing a text response and synthesizing corresponding audio.
    
    Validates the provided token and required fields, generates a character response using the LLM, synthesizes audio for the response, encodes the audio in base64, and returns both the generated text and audio data.
...
async def health_check(request: Request):
    """
    Health check endpoint that reports the readiness status of the LLM and TTS subsystems.
    
    Returns:
        dict: A JSON object containing the overall status ("ok" or "degraded"), the Actor ID, readiness flags for LLM and TTS, and a detail message if degraded.
    """
    client: CharacterClient = request.app.state.character_client_instance
    if not client:
...
async def _heartbeat_task_runner(client: CharacterClient):
    """
    Periodically sends heartbeat signals for the given CharacterClient instance to maintain server connectivity.
    
    Runs indefinitely, sleeping for a configured interval between heartbeats.
    """
    while True:
...
def start_heartbeat_task(client: CharacterClient):
    """
    Starts the asynchronous heartbeat task for the given character client if it is not already running.
    
    This ensures that the client periodically sends heartbeat signals to the server to maintain its registration status.
    """
...
def initialize_character_client(token: str, Actor_id: str, server_url: str, client_port: int):
    """
    Initializes and registers a CharacterClient instance for the FastAPI app if not already present.
    
    This function ensures required directories exist, asynchronously creates and initializes the CharacterClient, and schedules the heartbeat background task. If a client is already initialized, it does nothing.
    """
    global _heartbeat_task_instance
    if not hasattr(app.state, 'character_client_instance') or app.state.character_client_instance is None:
        ...
        async def _init():
            ...
            This function creates and stores a CharacterClient in the FastAPI app state, prints initialization status, and ensures the heartbeat background task is started for the client.
            """
