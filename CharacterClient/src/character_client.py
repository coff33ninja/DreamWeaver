...
class CharacterClient:
    def __init__(self, token: str, Actor_id: str, server_url: str, client_port: int):
        ...
        This constructor sets up the basic attributes for the client, but does not perform any network or blocking operations. Asynchronous initialization, including server registration and subsystem setup, must be completed by calling `async_init()` after construction.
    ...
    async def async_init(self):
        ...
        This method offloads blocking operations to background threads to avoid blocking the event loop. If registration or trait fetching fails, default values are used to ensure continued operation.
        """
        def _register():
    ...
    def _download_reference_audio_blocking(self):
        ...
        If the character traits or reference audio filename are missing, the operation is skipped. On failure, the local reference audio path is set to None.
        """
        if not self.character:
    ...
    def _fetch_traits_blocking(self):
        ...
        Returns:
            dict | None: The character traits as a dictionary if successful, otherwise None.
        """
        try:
    ...
    def _register_with_server_blocking(self, max_retries=DEFAULT_MAX_RETRIES, base_delay=DEFAULT_BASE_DELAY_SECONDS) -> bool:
        ...
        """
        Register the client with the server using a blocking HTTP POST request with retries and exponential backoff.
        
        Parameters:
        	max_retries (int): Maximum number of retry attempts if registration fails.
    ...
    async def send_heartbeat_async(self, max_retries=DEFAULT_MAX_RETRIES, base_delay=DEFAULT_BASE_DELAY_SECONDS) -> bool:
        ...
        def _blocking_heartbeat_call():
            """
            Send a blocking POST request with a JSON payload to the specified URL for heartbeat signaling.
    ...
    async def generate_response_async(self, narration: str, character_texts: dict) -> str:
        """
        Asynchronously generates a character-specific response using the LLM and submits training data for fine-tuning.
        
        Parameters:
            narration (str): The narration context to include in the prompt.
        ...
        def _save_training_data_blocking():
            """
            Sends training data to the server for storage using a blocking HTTP POST request.
            """
    ...
    async def synthesize_audio_async(self, text_to_synthesize: str) -> str | None:
        """
        Asynchronously generates an audio file from the provided text using the character's TTS configuration.
        
        Parameters:
            text_to_synthesize (str): The text to convert to speech.
        
        Returns:
            str | None: The path to the generated audio file, or None if TTS is not initialized.
        """
...
async def handle_character_generation_request(data: dict, request: Request):
    """
    Handles POST requests to generate a character response and synthesize corresponding audio.
    
    Validates the provided token and narration, generates a character response using the LLM, synthesizes audio for the response, encodes the audio in base64, and returns both the generated text and audio data.
...
async def health_check(request: Request):
    """
    Handles the `/health` endpoint, returning the readiness status of the character client and its LLM and TTS subsystems.
    
    Returns:
        dict: A JSON object containing the overall status, actor ID, readiness flags for LLM and TTS, and a detail message if degraded.
    """
    client: CharacterClient = request.app.state.character_client_instance
    if not client:
...
async def _heartbeat_task_runner(client: CharacterClient):
    ...
    Continuously waits for a fixed interval and then calls the client's asynchronous heartbeat method to maintain server connectivity.
    """
    while True:
...
def initialize_character_client(token: str, Actor_id: str, server_url: str, client_port: int):
    """
    Initializes and asynchronously creates a CharacterClient instance if one does not already exist.
    
    If no client is present, this function sets up required directories, creates and initializes the CharacterClient asynchronously, stores it in the FastAPI app state, and starts the heartbeat task. If a client is already initialized, it logs a message and takes no further action.
    
    Parameters:
        token (str): Authentication token for the character client.
        Actor_id (str): Unique identifier for the character.
        server_url (str): URL of the server to register and communicate with.
        client_port (int): Port number for the client instance.
    """
    global _heartbeat_task_instance
    if not hasattr(app.state, 'character_client_instance') or app.state.character_client_instance is None:
        ...
        async def _init():
            """
            Asynchronously initializes the CharacterClient instance, stores it in the FastAPI app state, and starts the heartbeat task if not already running.
            
            This function creates and initializes a CharacterClient with the provided configuration, updates the application state, logs readiness of LLM and TTS subsystems, and ensures the heartbeat task is scheduled for ongoing server communication.
            """
