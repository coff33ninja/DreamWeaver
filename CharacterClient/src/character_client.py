...
class CharacterClient:
    def __init__(self, token: str, Actor_id: str, server_url: str, client_port: int):
        """
        Initialize a CharacterClient instance with the specified authentication and connection parameters.
        
        This constructor sets up the basic attributes for the client, but does not perform any network operations or resource initialization. To complete setup, call the asynchronous `async_init()` method after construction.
        """
        self.token = token
    ...
    async def create(cls, token: str, Actor_id: str, server_url: str, client_port: int):
        ...
        Parameters:
            token (str): Authentication token for the character client.
            Actor_id (str): Unique identifier for the character actor.
            server_url (str): URL of the server to register and communicate with.
            client_port (int): Port number for the client service.
        
        Returns:
            CharacterClient: An initialized CharacterClient instance ready for use.
        """
    ...
    async def async_init(self):
        ...
        """
        Asynchronously initializes the CharacterClient by registering with the server, fetching character traits, downloading reference audio if needed, and setting up TTS and LLM components.
        
        This method performs all blocking operations in background threads to avoid blocking the event loop. If registration or trait fetching fails, it falls back to default character settings.
        """
        def _register():
        ...
        def _fetch():
            ...
            Returns:
                dict or None: The character traits dictionary if successful, otherwise None.
            """
        ...
        def _download():
            ...
            Returns:
                The result of the blocking reference audio download operation, or None if no download is needed.
            """
    ...
    def _download_reference_audio_blocking(self):
        ...
        """
        Downloads the character's reference audio file from the server and saves it locally if not already present.
        
        If the character traits or reference audio filename are missing, the method exits without downloading. The downloaded file is saved to a sanitized path based on the actor ID and filename. If the file already exists and is non-empty, it is not re-downloaded. On failure, the local reference audio path is set to None.
        """
        if not self.character:
    ...
    def _fetch_traits_blocking(self):
        ...
        Returns:
            dict or None: The character traits as a dictionary if successful, or None if the request fails.
        """
        try:
    ...
    def _register_with_server_blocking(self, max_retries=DEFAULT_MAX_RETRIES, base_delay=DEFAULT_BASE_DELAY_SECONDS) -> bool:
        ...
        """
        Register the client with the server using a blocking HTTP POST request with retry logic.
        
        Attempts to register the client by sending its actor ID, token, and port to the server's `/register` endpoint. Retries the request on failure with exponential backoff.
        
        Parameters:
            max_retries (int): Maximum number of retry attempts before giving up.
            base_delay (int): Initial delay in seconds before retrying, doubled after each failed attempt.
    ...
    async def send_heartbeat_async(self, max_retries=DEFAULT_MAX_RETRIES, base_delay=DEFAULT_BASE_DELAY_SECONDS) -> bool:
        ...
        def _blocking_heartbeat_call():
            """
            Send a blocking POST request to the specified URL with the given payload and a 5-second timeout.
    ...
    async def generate_response_async(self, narration: str, character_texts: dict) -> str:
        ...
        Parameters:
            narration (str): The narrator's input text.
            character_texts (dict): Dialogue or context from other characters.
        
        Returns:
            str: The generated response text from the character, or a status message if the LLM is not ready.
        """
        ...
        def _save_training_data_blocking():
            """
            Synchronously sends training data to the server for storage.
            
            This method posts the input prompt and generated output text to the server's training data endpoint, associating it with the current actor and authentication token.
            """
    ...
    async def synthesize_audio_async(self, text_to_synthesize: str) -> str | None:
        """
        Asynchronously synthesizes speech audio from the provided text using the initialized TTS engine.
        
        Parameters:
            text_to_synthesize (str): The text to convert to speech audio.
        
        Returns:
            str | None: The file path to the generated audio file, or None if TTS is not initialized.
        """
...
async def handle_character_generation_request(data: dict, request: Request):
    """
    Handles character generation requests by producing a text response and synthesizing corresponding audio.
    
    Receives a JSON payload with a token, narration, and optional character texts. Validates the token and narration, generates a character response using the LLM, synthesizes audio for the response, encodes the audio in base64, and returns both the generated text and audio data.
...
async def health_check(request: Request):
    """
    Handle health check requests for the character client service.
    
    Returns a JSON object indicating the readiness status of the LLM and TTS subsystems, along with the associated Actor ID. If the client is not initialized, raises a 503 HTTPException.
    """
    client: CharacterClient = request.app.state.character_client_instance
    if not client:
...
async def _heartbeat_task_runner(client: CharacterClient):
    """
    Periodically sends heartbeat signals from the client to the server at fixed intervals.
    
    Runs indefinitely, sleeping for the configured heartbeat interval between each signal.
    """
    while True:
...
def start_heartbeat_task(client: CharacterClient):
    """
    Starts the background heartbeat task for the given character client if it is not already running.
...
def initialize_character_client(token: str, Actor_id: str, server_url: str, client_port: int):
    """
    Initializes the CharacterClient singleton for the given actor and schedules the heartbeat background task.
    
    If the client is not already initialized, this function creates and asynchronously initializes a CharacterClient instance with the provided credentials and configuration, stores it in the FastAPI app state, and starts the heartbeat coroutine. If the client is already initialized, it does nothing.
    """
    global _heartbeat_task_instance
    if not hasattr(app.state, 'character_client_instance') or app.state.character_client_instance is None:
        ...
        async def _init():
            """
            Asynchronously initializes the CharacterClient instance, stores it in the FastAPI app state, and schedules the heartbeat background task if not already running.
            
            This function creates and initializes the character client with the provided configuration, logs the readiness of LLM and TTS subsystems, and ensures the heartbeat task is started to maintain server communication.
            """
