...
class TestConfigPaths:
    ...
    def test_default_paths_structure(self):
        ...
        assert config.DEFAULT_CLIENT_MODELS_PATH.startswith(
            config.DEFAULT_CLIENT_DATA_PATH
        )
        ...
        for path_const in path_constants:
            if hasattr(config, path_const):
                ...
                assert os.path.isabs(
                    path_value
                ), f"{path_const} should be absolute path"
...
class TestDirectoryCreation:
    ...
    def test_ensure_directories_handles_permission_errors(self, mock_makedirs):
        ...
        try:
            ...
            pytest.fail(
                "ensure_client_directories should handle PermissionError gracefully"
            )
...
class TestConfigEdgeCases:
    ...
    def test_config_with_special_characters_in_paths(self):
        ...
        special_chars_path = os.path.join(
            tempfile.gettempdir(), "test-path_with.special@chars"
        )

        with patch.dict(
            os.environ, {"DREAMWEAVER_CLIENT_DATA_PATH": special_chars_path}
        ):
            import importlib
...
class TestConfigSecurity:
    ...
    def test_config_path_traversal_prevention(self):
        ...
        for traversal_path in traversal_attempts:
            with patch.dict(
                os.environ, {"DREAMWEAVER_CLIENT_DATA_PATH": traversal_path}
            ):
                import importlib
    ...
    def test_directory_creation_performance(self):
        ...
        try:
            ...
            for i in range(10):
                ...
                with patch.dict(
                    os.environ, {"DREAMWEAVER_CLIENT_DATA_PATH": test_path}
                ):
                    config.ensure_client_directories()
