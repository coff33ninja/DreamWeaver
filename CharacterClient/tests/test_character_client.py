...
class TestCharacterClientSecurityScenarios(unittest.TestCase):
    ...
    def test_response_header_injection_protection(self, mock_get):
        ...
        result = self.client.get_character(1)
        self.assertEqual(result["id"], 1)
        class TestHandleConfigUpdate(IsolatedAsyncioTestCase):
            async def asyncSetUp(self):
                self.client = CharacterClient(token="test_token", Actor_id="test_actor", server_url="http://test.com", client_port=8080)
                self.client.tts = MagicMock()
            ...
            async def test_invalid_log_level(self):
                payload = {"log_level": "INVALID"}
                with self.assertLogs("dreamweaver_client", level="WARNING") as log:
                    await self.client._handle_config_update(payload)
                self.assertIn("Invalid log level received", log.output[0])

            @patch("CharacterClient.TTSManager")
            async def test_tts_reinitialization(self, mock_tts_manager):
                payload = {
                    "tts_service_name": "xttsv2",
                    "tts_model_name": "new_model",
                    "tts_language": "fr"
                }
                mock_tts_instance = mock_tts_manager.return_value
                ...
                self.assertTrue(self.client.tts.is_initialized)

            async def test_no_tts_changes(self):
                payload = {"log_level": "INFO"}
                self.client.tts.is_initialized = True

                await self.client._handle_config_update(payload)

                self.assertTrue(self.client.tts.is_initialized)  # Ensure TTSManager was not reinitialized
