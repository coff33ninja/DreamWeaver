...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=int(
            os.getenv("CLIENT_PORT", 8001)
        ),  # Default to 8001 to avoid conflict with server
    ...
    logger.info(
        f"Parsed arguments: Server URL='{args.server_url}', Actor_ID='{args.Actor_id}', Client Host='{args.client_host}', Client Port='{args.client_port}' Token Provided: {'Yes' if args.token else 'No'}"
    )

    if args.token is None:
        logger.error(
            "Client token not provided. Token is required via --token argument or CLIENT_TOKEN environment variable. Client will not start."
        )
        return
    ...
    if (
        hasattr(app.state, "character_client_instance")
        and app.state.character_client_instance
    ):
        start_heartbeat_task(
            app.state.character_client_instance
        )  # This function should also use logging
        logger.info(
            f"Client '{args.Actor_id}' API server starting on {args.client_host}:{args.client_port}"
        )
        ...
        logger.error(
            f"Failed to initialize CharacterClient for {args.Actor_id}. Client will not start."
        )
