...
*   **Enhanced Authentication**: Clients perform a handshake after registration to exchange their primary token for a short-lived session token, which is used for subsequent HTTP communication, improving security.
*   **Dynamic Client Configuration**: Server can push configuration updates (e.g., TTS settings, log levels) to connected clients in real-time via WebSockets.
...
*   **Interactive Gradio Interface**: A central web UI for:
    ...
    *   Creating and configuring characters (name, personality, voice, LLM model, Actor assignment), including remote configuration of clients.
...
*   **Clients (PC2, PC3, ...)**: Character nodes.
    ...
    *   Registers with the central server, providing its `Actor_id`, `token`, and listening `client_port`.
...
│       │       ├── piper/
│       │       └── reference_voices/
│       ├── logs/               # For client.log (see Client-Side Data Storage)
│       └── temp_audio/         # Temporary audio files synthesized by client
│
├── SERVER/logs/              # For server.log (created automatically)
│
...
*   **`models/llm/adapters/[Actor_id]/[model_name]/`**: LoRA adapters for client-specific fine-tuned LLMs.
*   **`models/llm/training_data_local/[Actor_id]/`**: JSON samples of training data (input/output pairs) saved by the client's LLM `fine_tune` placeholder.
*   **`models/tts/piper/[model_name]/`**: Piper TTS voice model files.
*   **`models/tts/reference_voices/`**: Downloaded reference audio files for XTTSv2 voice cloning, prefixed with `Actor_id`.
...
*   **`logs/client.log`**: Rotating log file for client-side activity and errors. Location is `[CLIENT_DATA_PATH]/logs/client.log`.
...
Both the Server and CharacterClient now implement structured logging to console and rotating files:
*   **Server Logs**: Found in `DreamWeaver/SERVER/logs/server.log`.
*   **Client Logs**: Found in `DreamWeaver/CharacterClient/data/logs/client.log` (path configurable via `DREAMWEAVER_CLIENT_DATA_PATH`).

These logs provide detailed operational information and are crucial for troubleshooting.
...
4.  **Launch Client (Command-Line):**
    ...
    ```bash
    python main.py --Actor_id <YourClientID> --token <ClientToken> --server_url <ServerURL> [options]
    ```
    **Required Arguments:**
    *   `--Actor_id <YourClientID>`: E.g., `Actor2`. Must match ID from Gradio UI. (Env: `CLIENT_Actor_ID`)
    *   `--token <ClientToken>`: Token from Gradio UI for this `Actor_id`. (Env: `CLIENT_TOKEN`)
    ...
    **Example:**
    `python main.py --Actor_id Actor2 --token xyz789 --server_url http://192.168.1.100:8000 --client_port 8002`
...
1.  Start Server.
2.  Use Gradio UI to create characters for the server (e.g., `Actor1` or a narrator identity) and any clients (e.g., `Actor2`, `Actor3`). Note client tokens.
3.  Start each `CharacterClient` with its `Actor_id`, `token`, and `server_url`. Ensure unique `client_port` if on same host.
